...
if IsDuplicityVersion() then
    ...
    function Framework.qb.SendLog(message)
        if Config.EnableLogs then
            TriggerEvent('nmsh-log:server:CreateLog', 'pshousing', 'Housing System', 'blue', message)
        end
    end
...
Framework.qb = {
    ...
    AddEntrance = function(coords, size, heading, propertyId, enter, raid, showcase, showData, targetName)
        ...
        exports["nmsh-target"]:AddBoxZone(
            targetName,
    ...
    AddApartmentEntrance = function(coords, size, heading, apartment, enter, seeAll, seeAllToRaid, targetName)
        exports['nmsh-target']:AddBoxZone(targetName, vector3(coords.x, coords.y, coords.z), size.x, size.y, {
            name = targetName,
            heading = heading,
            debugPoly = Config.DebugMode,
            minZ = coords.z - 1.0,
    ...
    RemoveTargetZone = function(targetName)
        exports["nmsh-target"]:RemoveZone(targetName)
    end,

    AddRadialOption = function(id, label, icon, _, event, options)
        exports['nmsh-radialmenu']:AddOption({
            id = id,
            title = label,
            icon = icon,
            type = 'client',
    ...
    RemoveRadialOption = function(id)
        exports['nmsh-radialmenu']:RemoveOption(id)
    end,
    ...
    RemoveTargetEntity = function (entity)
        exports["nmsh-target"]:RemoveTargetEntity(entity)
    end,
