...
AddEventHandler("ps-housing:server:registerProperty", function (propertyData) -- triggered by realtor job
    ...
    if propertyData.apartment then
        ...
        if result and result[1] then
            ...
            TriggerClientEvent("nmsh-clothes:client:CreateFirstCharacter", src)
...
-- If player has an existing apartment from nmsh-apartments, it will transfer the items over to the new apartment stash
RegisterNetEvent("ps-housing:server:createApartmentStash", function(citizenId, propertyId)
    ...
    TriggerEvent('nmsh-inventory:server:SaveStashItems', stashId, items)
end)
