...
GetJobMoney = function(job)
	...
	elseif GetResourceState('nmsh-management') == 'started' then
		return exports['nmsh-management']:GetAccount(job)
...
RemoveJobMoney = function(job,amount)
    ...
    elseif GetResourceState('es_extended') == 'started' then
		...
		return exports['nmsh-management']:RemoveMoney(job,amount)
    end
end