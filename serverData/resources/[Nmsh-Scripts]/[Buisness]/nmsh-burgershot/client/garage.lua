	...
	while true do
		...
		for k, v in pairs(Config.GaragePedLocations) do
			...
			if dist < 40 and not pedspawned then
				TriggerEvent('nmsh-burgershot:spawn:ped', v.coords)
				pedspawned = true
			end
			if dist >= 35 then
				pedspawned = false
...
RegisterNetEvent('nmsh-burgershot:spawn:ped')
AddEventHandler('nmsh-burgershot:spawn:ped',function(coords)
	local hash = `ig_floyd`
...
RegisterNetEvent('nmsh-burgershot:garage')
AddEventHandler('nmsh-burgershot:garage', function(bs)
...
RegisterNetEvent('nmsh-burgershot:storecar')
AddEventHandler('nmsh-burgershot:storecar', function()
...
RegisterNetEvent('garage:BurgerShotGarage', function()
    exports['nmsh-menu']:openMenu({
        ...
        },
        {
            ...
            params = {
                event = "nmsh-burgershot:storecar",
                args = {
