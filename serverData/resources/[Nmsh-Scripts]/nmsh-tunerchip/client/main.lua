...
RegisterNUICallback('save', function(data, cb)
    QBCore.Functions.TriggerCallback('nmsh-tunerchip:server:HasChip', function(HasChip)
        if HasChip then
            ...
            TriggerServerEvent('nmsh-tunerchip:server:TuneStatus', QBCore.Functions.GetPlate(veh), true)
        end
        cb('ok')
    end, data)
end)

RegisterNetEvent('nmsh-tunerchip:client:TuneStatus', function()
    local ped = PlayerPedId()
    ...
    if vehModel ~= 0 then
        QBCore.Functions.TriggerCallback('nmsh-tunerchip:server:GetStatus', function(status)
            if status then
...
RegisterNetEvent('nmsh-tunerchip:client:openChip', function()
    local ped = PlayerPedId()
