...
QBCore.Functions.CreateUseableItem('police_badge', function(source, item)
    ...
    TriggerClientEvent('dio_pdbadge:client:useBadge', source, item)
end)


RegisterNetEvent('dio_pdbadge:server:createBadge', function(name, callsign, rank, url, dept)
    local src = source
...
RegisterNetEvent('dio_pdbadge:server:showBadge', function(player, item)
    TriggerClientEvent('dio_pdbadge:client:ShowBadge', player, item)
end)
