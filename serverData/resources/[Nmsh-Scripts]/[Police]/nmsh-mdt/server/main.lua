...
RegisterServerEvent("nmsh-mdt:dispatchStatus", function(bool)
...
RegisterNetEvent("nmsh-mdt:server:OnPlayerUnload", function()
...
RegisterNetEvent("nmsh-mdt:server:ToggleDuty", function()
    local src = source
    local player = QBCore.Functions.GetPlayer(src)
...
QBCore.Commands.Add("mdtleaderboard", "Show MDT leaderboard", {}, false, function(source, args)
    ...
    sendToDiscord(16753920, "MDT Leaderboard", leaderboard_message, "nmsh-mdt | Made by Project Sloth")
    TriggerClientEvent('QBCore:Notify', source, "MDT leaderboard sent to Discord!", 'success')
end)

RegisterNetEvent("nmsh-mdt:server:ClockSystem", function()
    local src = source
    local PlayerData = GetPlayerData(src)
    local time = os.date("%Y-%m-%d %H:%M:%S")
    local firstName = PlayerData.charinfo.firstname:sub(1,1):upper()..PlayerData.charinfo.firstname:sub(2)
...
RegisterNetEvent('mdt:server:openMDT', function()
	...
	local calls = exports['nmsh-dispatch']:GetDispatchCalls()
					...
					if impoundInfo.impoundActive then
						...
						if vehicle.impoundid ~= nil then
							...
							if result then
								...
								result.plate = plate
								TriggerClientEvent('nmsh-mdt:client:TakeOutImpound', src, result)
							end
...
local function giveCitationItem(src, citizenId, fine, incidentId)
	...
	if Config.QBManagementUse then 
		exports['nmsh-management']:AddMoney(Officer.PlayerData.job.name, fine) 
	end
