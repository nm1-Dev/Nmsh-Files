...
CreateThread(function()
    if GetResourceState('nmsh-dispatch') == 'started' then
        TriggerServerEvent("nmsh-mdt:dispatchStatus", true)
    end
end)
...
RegisterNetEvent('QBCore:Client:OnPlayerUnload', function()
    TriggerServerEvent("nmsh-mdt:server:OnPlayerUnload")
    PlayerData = {}
end)
...
RegisterNetEvent("QBCore:Client:SetDuty", function(job, state)
    if AllowedJob(job) then
        TriggerServerEvent("nmsh-mdt:server:ToggleDuty")
	TriggerServerEvent("nmsh-mdt:server:ClockSystem")
...
RegisterNUICallback("sendToCommunityService", function(data, cb)
    ...
    if sentence > 0 then
        TriggerServerEvent("nmsh-communityservice:server:StartCommunityService", targetSourceId, sentence)
    end
end)
...
RegisterNUICallback("removeCallBlip", function(data, cb)
    TriggerEvent('nmsh-dispatch:client:removeCallBlip', data.callid)
    cb(true)
end)
...
RegisterNetEvent('nmsh-mdt:client:selfregister')
AddEventHandler('nmsh-mdt:client:selfregister', function()
...
if Config.UseWolfknightRadar == true then
    RegisterNetEvent("nmsh-mdt:client:trafficStop")
    AddEventHandler("nmsh-mdt:client:trafficStop", function()
        ...
        if not coolDown then
            if data.veh ~= nil and data.veh ~= 0 then
                ...
                exports["nmsh-dispatch"]:CustomAlert({
                    coords = {
                        x = currentPos.x,
                        y = currentPos.y,
                        z = currentPos.z
