...
RegisterNetEvent('nmsh-trashsearch:client:progressbar', function(itemType)
    ...
    QBCore.Functions.Progressbar("trash_find", "Dumpster Diving", 9000, false, true, {
        ...
        if Config.RewardTypes[itemType].type == "item" then
            ...
            local amount = token
            TriggerServerEvent('nmsh-trashsearch:server:recieveItem', amount)
        elseif Config.RewardTypes[itemType].type == "money" then
            QBCore.Functions.Notify("You found some cash", "success")
            QBCore.Functions.Notify("You smell like shit")
            local token = math.random(1, 9999)
            local amount = token
            TriggerServerEvent('nmsh-trashsearch:server:givemoney', math.random (21, 39), amount)
