            ...
            if (QB.Phone.Data.currentApplication == null) {
                ...
                if (PressedApplication == "twitter") {
                    if (QB.Phone.Data.IsOpen) {
                        $.post('https://nmsh-phone/GetTweets', JSON.stringify({}), function(Tweets){
                    ...
                    $.post('https://nmsh-phone/GetBankContacts', JSON.stringify({}), function(contacts){
                        QB.Phone.Functions.LoadContactsWithNumber(contacts);
                    });
                    $.post('https://nmsh-phone/GetInvoices', JSON.stringify({}), function(invoices){
                        QB.Phone.Functions.LoadBankInvoices(invoices);
                    });
                } else if (PressedApplication == "whatsapp") {
                    $.post('https://nmsh-phone/GetWhatsappChats', JSON.stringify({}), function(chats){
                        QB.Phone.Functions.LoadWhatsappChats(chats);
                    });
                } else if (PressedApplication == "phone") {
                    $.post('https://nmsh-phone/GetMissedCalls', JSON.stringify({}), function(recent){
                        QB.Phone.Functions.SetupRecentCalls(recent);
                    });
                    $.post('https://nmsh-phone/ClearGeneralAlerts', JSON.stringify({
                        app: "phone"
                    }));
                } else if (PressedApplication == "mail") {
                    $.post('https://nmsh-phone/GetMails', JSON.stringify({}), function(mails){
                    ...
                    $.post('https://nmsh-phone/LoadAdverts', JSON.stringify({}), function(Adverts){
                        QB.Phone.Functions.RefreshAdverts(Adverts);
                    })
                } else if (PressedApplication == "garage") {
                    $.post('https://nmsh-phone/SetupGarageVehicles', JSON.stringify({}), function(Vehicles){
                        SetupGarageVehicles(Vehicles);
                    })
                } else if (PressedApplication == "racing") {
                    $.post('https://nmsh-phone/GetAvailableRaces', JSON.stringify({}), function(Races){
                        SetupRaces(Races);
                    });
                } else if (PressedApplication == "houses") {
                    $.post('https://nmsh-phone/GetPlayerHouses', JSON.stringify({}), function(Houses){
                        SetupPlayerHouses(Houses);
                    });
                    $.post('https://nmsh-phone/GetPlayerKeys', JSON.stringify({}), function(Keys){
                    ...
                    $.post('https://nmsh-phone/GetAvailableTaxiDrivers', JSON.stringify({}), function(data){
                        SetupTaxiDrivers(data);
                    });
                }
                else if (PressedApplication == "gallery") {
                    $.post('https://nmsh-phone/GetGalleryData', JSON.stringify({}), function(data){
                    ...
                    $.post('https://nmsh-phone/GetJobs', JSON.stringify({}), function(data){
                        LoadEmploymentApp(data)
                    });
                } else if (PressedApplication == "debt") {
                    $.post('https://nmsh-phone/GetPlayersDebt', JSON.stringify({}), function(data){
                        LoadDebtJob(data);
                    });
                }
                else if (PressedApplication == "gopro") {
                    $.post('https://nmsh-phone/SetupGoPros', JSON.stringify({}), function(Cams){
                        SetupGoPros(Cams);
                    })
                }
                    ...
                    $.post('https://nmsh-phone/GetChatRooms', JSON.stringify({}), function(ChatRooms){
...
$(document).on('click', '.mykeys-key', function(e){
    ...
    $.post('https://nmsh-phone/SetHouseLocation', JSON.stringify({
        HouseData: KeyData
    }))
});
...
$(document).on('click', '.phone-take-camera-button', function(event){
    ...
    $.post('https://nmsh-phone/TakePhoto', JSON.stringify({}),function(url){
        // setUpCameraApp(url)
    })
...
$(document).on('click', '.phone-silent-button', function(event){
    event.preventDefault();
    $.post('https://nmsh-phone/phone-silent-button', JSON.stringify({}),function(Data){
        if(Data){
...
QB.Phone.Notifications.Custom.Add = function(icon, title, text, color, timeout, accept, deny) {
    $.post('https://nmsh-phone/HasPhone', JSON.stringify({}), function(HasPhone){
        if (HasPhone) {
...
$(document).on('click', ".notification-accept", function() {
    $.post('https://nmsh-phone/AcceptNotification', JSON.stringify({})),
    QB.Phone.Animations.TopSlideUp(".phone-notification-container-new", 150, -8);
    QB.Phone.Notifications.Timeout = null
...
$(document).on('click', ".notification-deny", function() {
    $.post('https://nmsh-phone/DenyNotification', JSON.stringify({})),
    QB.Phone.Notifications.Timeout = null
