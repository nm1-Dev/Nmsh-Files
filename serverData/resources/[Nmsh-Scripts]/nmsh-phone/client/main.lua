...
exports["nmsh-target"]:AddTargetModel(PublicPhoneobject, {
    options = {
        {
            type = "client",
            event = "nmsh-phone:client:publocphoneopen",
...
local function LoadPhone()
    QBCore.Functions.TriggerCallback('nmsh-phone:server:GetPhoneData', function(pData)
...
local function CancelCall()
    TriggerServerEvent('nmsh-phone:server:CancelCall', PhoneData.CallData)
    ...
    TriggerServerEvent('nmsh-phone:server:SetCallState', false)
...
local function CallContact(CallData, AnonymousCall)
    ...
    TriggerServerEvent('nmsh-phone:server:CallContact', PhoneData.CallData.TargetData, PhoneData.CallData.CallId, AnonymousCall)
    TriggerServerEvent('nmsh-phone:server:SetCallState', true)
...
local function AnswerCall()
    if (PhoneData.CallData.CallType == "incoming" or PhoneData.CallData.CallType == "outgoing") and PhoneData.CallData.InCall and not PhoneData.CallData.AnsweredCall then
        ...
        TriggerServerEvent('nmsh-phone:server:AnswerCall', PhoneData.CallData)
...
RegisterNUICallback('SetBackground', function(data, cb)
    ...
    TriggerServerEvent('nmsh-phone:server:SaveMetaData', PhoneData.MetaData)
    cb('ok')
end)
...
RegisterNUICallback('AddNewContact', function(data, cb)
    ...
    TriggerServerEvent('nmsh-phone:server:AddNewContact', data.ContactName, data.ContactNumber)
end)
...
RegisterNUICallback('EditContact', function(data, cb)
    ...
    Wait(100)
    cb(PhoneData.Contacts)
    TriggerServerEvent('nmsh-phone:server:EditContact', NewName, NewNumber, OldName, OldNumber)
end)
...
RegisterNUICallback('FetchSearchResults', function(data, cb)
    QBCore.Functions.TriggerCallback('nmsh-phone:server:FetchResult', function(result)
        cb(result)
    end, data.input)
end)
...
RegisterNUICallback('DeleteContact', function(data, cb)
    ...
    TriggerServerEvent('nmsh-phone:server:RemoveContact', Name, Number)
end)
...
RegisterNUICallback('CallContact', function(data, cb)
    QBCore.Functions.TriggerCallback('nmsh-phone:server:GetCallState', function(CanCall, IsOnline)
        local status = {
            CanCall = CanCall,
            IsOnline = IsOnline,
...
RegisterNUICallback("TakePhoto", function(_, cb)
    ...
    while true do
        ...
        elseif IsControlJustPressed(1, 176) then
            QBCore.Functions.TriggerCallback("nmsh-phone:server:GetWebhook",function(hook)
                ...
                exports['screenshot-basic']:requestScreenshotUpload(tostring(hook), "files[]", function(uploadData)
                    ...
                    DestroyMobilePhone()
                    CellCamActivate(false, false)
                    TriggerServerEvent('nmsh-phone:server:addImageToGallery', image.attachments[1].proxy_url)
                    Wait(400)
...
RegisterNetEvent('nmsh-phone:client:AddRecentCall', function(data, time, type)
...
RegisterNetEvent('nmsh-phone:client:CancelCall', function()
    if PhoneData.CallData.CallType == "ongoing" then
        SendNUIMessage({
...
RegisterNetEvent('nmsh-phone:client:GetCalled', function(CallerNumber, CallId, AnonymousCall)
    local RepeatCount = 0
    local CallData = {
        number = CallerNumber,
            ...
            if not PhoneData.CallData.AnsweredCall then
                if RepeatCount + 1 ~= Config.CallRepeats + 1 then
                    if PhoneData.CallData.InCall then
                        ...
                        TriggerServerEvent('nmsh-phone:server:AddRecentCall', "missed", CallData)
                        break
                    end
                    Wait(Config.RepeatTimeout)
                else
...
RegisterNetEvent('nmsh-phone:client:AnswerCall', function()
    if (PhoneData.CallData.CallType == "incoming" or PhoneData.CallData.CallType == "outgoing") and PhoneData.CallData.InCall and not PhoneData.CallData.AnsweredCall then
        PhoneData.CallData.CallType = "ongoing"
        PhoneData.CallData.AnsweredCall = true
        PhoneData.CallData.CallTime = 0
...
RegisterNetEvent('nmsh-phone:client:clearAppAlerts', function()
    Config.PhoneApplications["phone"].Alerts = 0
    SendNUIMessage({ action = "RefreshAppAlerts", AppData = Config.PhoneApplications })
end)
...
RegisterNetEvent('nmsh-phone:client:publocphoneopen',function()
    SetNuiFocus(true, true)
...
RegisterNUICallback('CanTransferMoney', function(data, cb)
    ...
    if (PlayerData.money.bank - amount) >= 0 then
        QBCore.Functions.TriggerCallback('nmsh-phone:server:CanTransferMoney', function(Transferd)
            if Transferd then
...
RegisterNetEvent('nmsh-phone:client:TransferMoney', function(amount, newmoney)
...
RegisterNetEvent("nmsh-phone-new:client:BankNotify", function(text)
    SendNUIMessage({
...
RegisterNetEvent('nmsh-phone:client:RemoveBankMoney', function(amount)
    if amount > 0 then
        SendNUIMessage({
            action = "PhoneNotification",
            PhoneNotify = {
...
RegisterNetEvent('nmsh-phone:client:GiveContactDetails', function()
    local player, distance = QBCore.Functions.GetClosestPlayer()
    if player ~= -1 and distance < 2.5 then
        local PlayerId = GetPlayerServerId(player)
        TriggerServerEvent('nmsh-phone:server:GiveContactDetails', PlayerId)
    else
        QBCore.Functions.Notify("No one nearby!", "error")
    end
end)

RegisterNetEvent("nmsh-phone:client:giveContactRequest", function(contactInfo)
    local success = exports['nmsh-phone']:PhoneNotification("CONTACT REQUEST", contactInfo.name..' contact request', 'fas fa-phone', '#b3e0f2', "NONE", 'fas fa-check-circle', 'fas fa-times-circle')
    if success then
        TriggerServerEvent('nmsh-phone:server:acceptContactRequest', contactInfo)
    end
end)

RegisterNetEvent('nmsh-phone:client:updateContactInfo', function(contactInfo)
    PhoneData.Contacts[#PhoneData.Contacts+1] = {
...
RegisterNetEvent('nmsh-phone:RefreshPhone', function()
    LoadPhone()
    SetTimeout(250, function()
        SendNUIMessage({
