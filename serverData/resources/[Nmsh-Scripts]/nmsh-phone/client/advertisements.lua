...
RegisterNUICallback('PostAdvert', function(data, cb)
    ...
    TriggerServerEvent('nmsh-phone:server:AddAdvert', data.message, url)
    cb("ok")
end)


RegisterNUICallback('FlagAdvert', function(data, cb)
    TriggerServerEvent('nmsh-phone:server:flagAdvert', data.number)
    cb("ok")
end)

RegisterNUICallback("DeleteAdvert", function(_, cb)
    TriggerServerEvent("nmsh-phone:server:DeleteAdvert")
    cb("ok")
end)
...
RegisterNetEvent('nmsh-phone:client:UpdateAdverts', function(Adverts, LastAd, src)
