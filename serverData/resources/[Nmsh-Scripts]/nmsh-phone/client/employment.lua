...
RegisterNUICallback('SendEmployeePayment', function(data, cb)
    ...
    TriggerServerEvent('nmsh-phone:server:SendEmploymentPayment', data.job, data.cid, data.amount)
    cb("ok")
end)
...
RegisterNUICallback('RemoveEmployee', function(data, cb)
    ...
    TriggerServerEvent('nmsh-phone:server:fireUser', data.job, data.cid)
...
RegisterNUICallback('ChangeRole', function(data, cb)
    ...
    TriggerServerEvent('nmsh-phone:server:gradesHandler', data.job, tostring(data.cid), data.grade)
    cb("ok")
end)
...
RegisterNUICallback('ClockIn', function(data, cb)
    ...
    TriggerServerEvent('nmsh-phone:server:clockOnDuty', data.job)
    cb("ok")
end)
...
RegisterNUICallback('HireFucker', function(data, cb)
    ...
    TriggerServerEvent('nmsh-phone:server:hireUser', data.job, data.stateid, data.grade)
...
RegisterNUICallback('ChargeMF', function(data, cb)
    ...
    TriggerServerEvent('nmsh-phone:server:ChargeCustomer', data.stateid, data.amount, data.note, data.job)
    cb("ok")
end)

RegisterNetEvent('nmsh-phone:client:JobsHandler', function(job, employees)
...
RegisterNetEvent('nmsh-phone:client:MyJobsHandler', function(job, jobTable, employees)
...
AddEventHandler('onResourceStart', function(resource)
    if resource == GetCurrentResourceName() then
        Wait(300)
        QBCore.Functions.TriggerCallback('nmsh-phone:server:GetMyJobs', function(employees, myShit)
