...
RegisterNetEvent('nmsh-phone:server:InvoiceHandler')
...
RegisterNetEvent('nmsh-phone:server:PayMyInvoice', function(society, amount, invoiceId, sendercitizenid, resource)
    local src = source
    ...
    if Player.PlayerData.money.bank >= amount then
        ...
        if SenderPly then
            TriggerClientEvent('nmsh-phone:client:CustomNotification', SenderPly.PlayerData.source,
        ...
        TriggerClientEvent('nmsh-phone:client:RemoveInvoiceFromTable', src, invoiceId)
        TriggerEvent("nmsh-phone:server:InvoiceHandler", true, amount, src, resource)

        exports.oxmysql:execute('DELETE FROM phone_invoices WHERE id = ?', {invoiceId})
    end
end)

RegisterNetEvent('nmsh-phone:server:DeclineMyInvoice', function(amount, invoiceId, sendercitizenid, resource)
    ...
    TriggerClientEvent('nmsh-phone:client:RemoveInvoiceFromTable', source, invoiceId)
    TriggerEvent("nmsh-phone:server:InvoiceHandler", false, amount, source, resource)
end)


RegisterNetEvent('nmsh-phone:server:CreateInvoice', function(billed, biller, amount)
    ...
    MySQL.Async.insert('INSERT INTO phone_invoices (citizenid, amount, society, sender, sendercitizenid) VALUES (?, ?, ?, ?, ?)',{
        ...
        if id then
            TriggerClientEvent('nmsh-phone:client:AcceptorDenyInvoice', billedCID.PlayerData.source, id, billerInfo.PlayerData.charinfo.firstname, billerInfo.PlayerData.job.name, billerInfo.PlayerData.citizenid, cash, resource)
        end
    end)
end)

QBCore.Functions.CreateCallback('nmsh-phone:server:GetInvoices', function(source, cb)
