...
RegisterNetEvent("nmsh-phone:server:sendPing", function(id)
    local src = source
    ...
    if Player.PlayerData.citizenid ~= Other.PlayerData.citizenid then
        TriggerClientEvent("nmsh-phone:client:sendNotificationPing", Shitter, info)
...
RegisterNetEvent("nmsh-phone:server:sendingPing", function(Other, Player, Name, OtherName)
    TriggerClientEvent('nmsh-phone:client:CustomNotification', Player, "PING", OtherName..' Accepted Your Ping!', 'fas fa-map-pin', '#b3e0f2', 7500)
    TriggerClientEvent("nmsh-phone:client:sendPing", Other, Name, GetEntityCoords(GetPlayerPed(Player)))
end)