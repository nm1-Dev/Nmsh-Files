...
local function notifyPlayer(src, message)
    ...
    TriggerClientEvent('nmsh-phone:client:CustomNotification', src,
        "Employment",
        message,
...
RegisterNetEvent('nmsh-phone:server:fireUser', function(Job, sCID)
    local src = source
    ...
    TriggerClientEvent("nmsh-phone:client:JobsHandler", -1, Job, CachedJobs[Job].employees)
    ...
    if Player.PlayerData.source then
        TriggerClientEvent('nmsh-phone:client:MyJobsHandler', Player.PlayerData.source, Job, nil, nil)
    end
end)

---- ** Can give any employee x amount of money out of the bank account, must be boss ** ----
RegisterNetEvent('nmsh-phone:server:SendEmploymentPayment', function(Job, CID, amount)
    local src = source
    ...
    if Config.RenewedBanking then
        ...
        if not exports['nmsh-management']:RemoveMoney(Job, amt) then return notifyPlayer(src, "Insufficient Funds...") end
    end
    Player.Functions.AddMoney('bank', amt)
end)

---- ** Player can hire someone aslong as they are boss within the group
RegisterNetEvent('nmsh-phone:server:hireUser', function(Job, id, grade)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    ...
    if hiredPlayer and CachedPlayers[CID] then
        ...
        TriggerClientEvent('nmsh-phone:client:MyJobsHandler', hiredPlayer.PlayerData.source, Job, CachedPlayers[CID][Job], CachedJobs[Job].employees)
    end
end)

---- ** Handles the changing of someone grade within the job ** ----

RegisterNetEvent('nmsh-phone:server:gradesHandler', function(Job, CID, grade)
    local src = source
    local srcPlayer = QBCore.Functions.GetPlayer(src)
    ...
    if Player and CachedPlayers[CID] then
        ...
        TriggerClientEvent('nmsh-phone:client:MyJobsHandler', Player.PlayerData.source, Job, CachedPlayers[CID][Job], CachedJobs[Job].employees)
    end
end)


RegisterNetEvent('nmsh-phone:server:clockOnDuty', function(Job)
    local src = source
    ...
    if CachedPlayers[CID][Job] and CachedJobs[Job].employees[CID] then
        ...
        TriggerClientEvent('nmsh-phone:client:clearAppAlerts', src)
    end
end)

---- Gets the client side cache for players ----
QBCore.Functions.CreateCallback("nmsh-phone:server:GetMyJobs", function(source, cb)
    if FirstStart then return end
...
RegisterNetEvent('nmsh-phone:server:ChargeCustomer', function(id, amount, notes, job)
    ...
    TriggerEvent('nmsh-phone:server:CreateInvoice', tonumber(id), src, amt)
end)

AddEventHandler('nmsh-phone:server:InvoiceHandler', function(paid, amount, source, resource)
    ...
    if paid then
        if amount == bills[source].amount then
            ...
            if Config.RenewedBanking then
                ...
                exports['nmsh-management']:AddMoney(bills[source].job, amount)
            end
