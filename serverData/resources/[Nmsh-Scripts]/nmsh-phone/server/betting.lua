...
local CasinoTable = {}
local BetNumber = 0
RegisterNetEvent('nmsh-phone:server:CasinoAddBet', function(data)
    BetNumber += 1
    CasinoTable[BetNumber] = {['Name'] = data.name, ['chanse'] = data.chanse, ['id'] = BetNumber}
    TriggerClientEvent('nmsh-phone:client:addbetForAll', -1, CasinoTable)
end)

local CasinoBetList = {}
local casino_status = true

RegisterNetEvent('nmsh-phone:server:BettingAddToTable', function(data)
    local src = source
...
RegisterNetEvent('nmsh-phone:server:DeleteAndClearTable', function()
    local src = source
    CasinoTable = {}
    CasinoBetList = {}
    BetNumber = 0
...
QBCore.Functions.CreateCallback('nmsh-phone:server:CheckHasBetTable', function(_, cb)
    cb(CasinoTable)
end)


RegisterNetEvent('nmsh-phone:server:casino_status', function()
    casino_status = not casino_status
end)

QBCore.Functions.CreateCallback('nmsh-phone:server:CheckHasBetStatus', function(_, cb)
    cb(casino_status)
end)

RegisterNetEvent('nmsh-phone:server:WineridCasino', function(data)
    local Winer = data.id
