...
RegisterNetEvent('nmsh-phone:server:AddAdvert', function(msg, url)
    local src = source
    ...
    TriggerClientEvent('nmsh-phone:client:UpdateAdverts', -1, Adverts, name, src)
end)

RegisterNetEvent('nmsh-phone:server:DeleteAdvert', function()
    local k = GetAdvertFromNumb(source)
    if not k then return end
    table.remove(Adverts, k)
    TriggerClientEvent('nmsh-phone:client:UpdateAdverts', -1, Adverts)
end)

RegisterNetEvent('nmsh-phone:server:flagAdvert', function(number)
    local src = source
