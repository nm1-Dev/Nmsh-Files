...
QBCore.Functions.CreateCallback('nmsh-phone:server:GetVehicleSearchResults', function(_, cb, search)
...
QBCore.Functions.CreateCallback('nmsh-phone:server:ScanPlate', function(source, cb, plate)
    local src = source
    local vehicleData
    if plate then
...
RegisterNetEvent('nmsh-phone:server:ClientScanPlate', function(plate)
    local src = source
    local vehicleData = {}
    if plate then
        local result = exports.oxmysql:executeSync('SELECT * FROM player_vehicles WHERE plate = ?', {plate})
