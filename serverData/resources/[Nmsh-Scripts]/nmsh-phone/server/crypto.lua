...
-- exports['nmsh-phone']:RemoveCrypto(Player, type, amount)
local function RemoveCrypto(src, type, amount)
    ...
    if (Crypto[type] - amount) >= 0 then
        ...
        TriggerClientEvent('nmsh-phone:client:UpdateCrypto', src)
        return true
    else
        return false
    end
...
-- exports['nmsh-phone']:hasEnough(Player, type, amount)
...
-- exports['nmsh-phone']:AddCrypto(Player, type, amount)
...
RegisterNetEvent('nmsh-phone:server:PurchaseCrypto', function(type, amount)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
RegisterNetEvent('nmsh-phone:server:SellCrypto', function(type, amount)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if not Player or not Player.PlayerData.metadata.crypto[type] then return end -- if the crypto dosnt exist
    local v = Config.CryptoCoins[GetConfig(type)]
...
RegisterNetEvent('nmsh-phone:server:ExchangeCrypto', function(type, amount, stateid)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    ...
    if Player.PlayerData.citizenid ~= Receiver.PlayerData.citizenid then
        if RemoveCrypto(src, type, amount) then
            ...
            TriggerClientEvent('nmsh-phone:client:CustomNotification', Receiver.PlayerData.source,
                "WALLET",
