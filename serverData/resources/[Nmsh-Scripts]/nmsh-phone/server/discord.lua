            ...
            for _, room in pairs(chatrooms) do
                ...
                if Player then
                    if Player.PlayerData.money.bank >= price then
                        ...
                        TriggerEvent('nmsh-phone:server:sendNewEventMail', room.room_owner_id, {
                            room.room_owner_id,
...
QBCore.Functions.CreateCallback('nmsh-phone:server:GetGroupChatMessages', function(_, cb, roomID)
...
QBCore.Functions.CreateCallback('nmsh-phone:server:SearchGroupChatMessages', function(source, cb, roomID, searchTerm)
    local src = source
    ...
    if isOwnerOfRoom(Player.PlayerData.citizenid, roomID) or isMemberOfRoom(Player.PlayerData.citizenid, roomID) then
        ...
        TriggerClientEvent('nmsh-phone:client:notification', src, 'Discord', 'You must be a member or room owner to search.')
        cb(false)
    end
end)

QBCore.Functions.CreateCallback('nmsh-phone:server:GetPinnedMessages', function(source, cb, roomID)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    ...
    if isOwnerOfRoom(Player.PlayerData.citizenid, roomID) or isMemberOfRoom(Player.PlayerData.citizenid, roomID) then
        ...
        TriggerClientEvent('nmsh-phone:client:notification', src, 'Discord', 'You must be a member or room owner to fetch that.')
        cb(false)
    end
end)


QBCore.Functions.CreateCallback('nmsh-phone:server:TryPinCode', function(_, cb, pinCode, roomID)
...
QBCore.Functions.CreateCallback('nmsh-phone:server:IsRoomOwner', function(source, cb, roomID)
...
RegisterNetEvent('nmsh-phone:server:SendGroupChatMessage', function(messageData, systemMessage, roomID)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if messageData and not systemMessage then
        if isOwnerOfRoom(Player.PlayerData.citizenid, roomID) or isMemberOfRoom(Player.PlayerData.citizenid, roomID) then
            ...
            TriggerClientEvent('nmsh-phone:client:RefreshGroupChat', -1, src, messageData)
            TriggerEvent("nmsh-log:server:CreateLog", "discord", "Message Posted (room: ".. messageData.room_id .. ", from: ".. Player.PlayerData.citizenid ..")", "blue", messageData.message)
        else
            TriggerClientEvent('nmsh-phone:client:notification', src, 'Discord', 'You must be a member or room owner to send messages.')
        end
    else
        ...
        TriggerClientEvent('nmsh-phone:client:RefreshGroupChat', -1, src, systemMessage)
    end
end)

QBCore.Functions.CreateCallback('nmsh-phone:server:JoinGroupChat', function(source, cb, updatedRooms, roomID)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if not isMemberOfRoom(Player.PlayerData.citizenid, roomID) then
        ...
        TriggerClientEvent('nmsh-phone:client:RefreshChatRooms', -1, ChatRooms)
        cb(true)
    end
end)

RegisterNetEvent('nmsh-phone:server:LeaveGroupChat', function(updatedRooms, roomID)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    local members = {}
...
RegisterNetEvent('nmsh-phone:server:ChangeRoomPin', function(updatedRooms, roomID, pin)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
RegisterNetEvent('nmsh-phone:server:DeactivateRoom', function(updatedRooms, roomID)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
QBCore.Functions.CreateCallback('nmsh-phone:server:PurchaseRoom', function(source, cb, price, roomData)
	local Player = QBCore.Functions.GetPlayer(source)
    if Player.PlayerData.money.bank >= price then
        ...
        TriggerClientEvent('nmsh-phone:client:RefreshChatRooms', -1, ChatRooms)
        TriggerEvent("nmsh-log:server:CreateLog", "discord", "Channel Created:  ".. roomData.room_name .."(id: ".. roomID.. ", by: "..Player.PlayerData.citizenid..")", "blue")
		cb(true)
	else
		cb(false)
	end
...
RegisterNetEvent('nmsh-phone:server:ToggleMessagePin', function(messageID, roomID)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
