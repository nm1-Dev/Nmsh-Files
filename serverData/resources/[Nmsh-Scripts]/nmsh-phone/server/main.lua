...
QBCore.Functions.CreateCallback('nmsh-phone:server:GetCallState', function(source, cb, ContactData)
...
QBCore.Functions.CreateCallback('nmsh-phone:server:GetPhoneData', function(source, cb)
    local src = source
...
QBCore.Functions.CreateCallback('nmsh-phone:server:FetchResult', function(_, cb, input)
...
QBCore.Functions.CreateCallback("nmsh-phone:server:GetWebhook",function(_, cb)
	cb(WebHook)
end)

-- Events
RegisterNetEvent('nmsh-phone:server:SetCallState', function(bool)
    local src = source
...
RegisterNetEvent('nmsh-phone:server:CallContact', function(TargetData, CallId, AnonymousCall)
    local src = source
    local Ply = QBCore.Functions.GetPlayer(src)
    local Target = QBCore.Functions.GetPlayerByPhone(tostring(TargetData.number))
    if not Target or not Ply then return end

    TriggerClientEvent('nmsh-phone:client:GetCalled', Target.PlayerData.source, Ply.PlayerData.charinfo.phone, CallId, AnonymousCall)
end)

RegisterNetEvent('nmsh-phone:server:EditContact', function(newName, newNumber, oldName, oldNumber)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
RegisterNetEvent('nmsh-phone:server:RemoveContact', function(Name, Number)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
RegisterNetEvent('nmsh-phone:server:AddNewContact', function(name, number)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
RegisterNetEvent('nmsh-phone:server:AddRecentCall', function(type, data)
    local src = source
    local Ply = QBCore.Functions.GetPlayer(src)
    ...
    TriggerClientEvent('nmsh-phone:client:AddRecentCall', src, data, label, type)

    local Target = QBCore.Functions.GetPlayerByPhone(data.number)
    if not Target then return end

    TriggerClientEvent('nmsh-phone:client:AddRecentCall', Target.PlayerData.source, {
...
RegisterNetEvent('nmsh-phone:server:GiveContactDetails', function(PlayerId)
    ...
    TriggerClientEvent('nmsh-phone:client:giveContactRequest', PlayerId, contactInfo)
end)

RegisterNetEvent('nmsh-phone:server:acceptContactRequest', function(contactInfo)
    ...
    TriggerClientEvent('nmsh-phone:client:updateContactInfo', src, contactInfo)
end)

RegisterNetEvent('nmsh-phone:server:CancelCall', function(ContactData)
    local Ply = QBCore.Functions.GetPlayerByPhone(tostring(ContactData.TargetData.number))
    if not Ply then return end
    TriggerClientEvent('nmsh-phone:client:CancelCall', Ply.PlayerData.source)
end)

RegisterNetEvent('nmsh-phone:server:AnswerCall', function(CallData)
    local Ply = QBCore.Functions.GetPlayerByPhone(CallData.TargetData.number)
    if not Ply then return end

    TriggerClientEvent('nmsh-phone:client:AnswerCall', Ply.PlayerData.source)
end)

RegisterNetEvent('nmsh-phone:server:SaveMetaData', function(MData)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if not Player then return end
