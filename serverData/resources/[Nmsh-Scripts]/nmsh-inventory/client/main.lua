...
RegisterNetEvent('inventory:client:OpenInventory', function(PlayerAmmo, inventory, other)
    if not IsEntityDead(PlayerPedId()) then
        QBCore.Functions.Progressbar("open_inventory", 'Open', 500, false, true, {
...
RegisterNUICallback('openProtectAble', function(cData, cb)
    ...
    Wait(100)
    openAnim()
    TriggerEvent("inventory:client:SetCurrentStash", "Protected_"..Player.citizenid)
    TriggerServerEvent("inventory:server:OpenInventory", "stash", "Protected_"..Player.citizenid, {
    maxweight = 30000, slots = 5})
    cb('ok')
end)
