    ...
    for _, station in pairs(toLoop) do
        if type(station) == 'table' then
            ...
            for _, station2 in pairs(station) do
                if Bank.PowerStations[station2].hit then hits2 += 1 end
                if hits2 == #station then return true end
            end
        else
            if Bank.PowerStations[station].hit then hits += 1 end
...
local function CheckStationHits()
    ...
    for k, v in pairs(Bank.CameraHits) do
            ...
            for _, cameraType in pairs(v.type) do
                if cameraType == 'police' then
                    if type(v.stationsToHitPolice) == 'table' then
                        ...
                        allStationsHitPolice = Bank.PowerStations[v.stationsToHitPolice].hit
                    end
                elseif cameraType == 'bank' then
                    if type(v.stationsToHitBank) == 'table' then
                        ...
                        allStationsHitBank = Bank.PowerStations[v.stationsToHitBank].hit
                    end
                end
            end
        else
    ...
    if table.type(policeHits) == 'array' then Bank.OnPoliceCameraHit(policeHits) end
...
local function AllStationsHit()
    local hit = 0
    for k in pairs(Bank.PowerStations) do
        if Bank.PowerStations[k].hit then
            hit += 1
        end
    end
    return hit >= Bank.HitsNeeded
end
...
local function IsNearPowerStation(coords, dist)
    for _, v in pairs(Bank.PowerStations) do
...
RegisterNetEvent('nmsh-bankrobbery:server:setBankState', function(bankId)
    ...
    if bankId == "paleto" then
        if Bank.BigBanks["paleto"]["isOpened"] or #(GetEntityCoords(GetPlayerPed(source)) - Bank.BigBanks["paleto"]["coords"]) > 2.5 then
    ...
    elseif bankId == "pacific" then
        if Bank.BigBanks["pacific"]["isOpened"] or #(GetEntityCoords(GetPlayerPed(source)) - Bank.BigBanks["pacific"]["coords"][2]) > 2.5 then
        ...
        if Bank.SmallBanks[bankId]["isOpened"] or #(GetEntityCoords(GetPlayerPed(source)) - Bank.SmallBanks[bankId]["coords"]) > 2.5 then
...
RegisterNetEvent('nmsh-bankrobbery:server:setLockerState', function(bankId, lockerId, state, bool)
    if bankId == "paleto" or bankId == "pacific" then
        if #(GetEntityCoords(GetPlayerPed(source)) - Bank.BigBanks[bankId]["lockers"][lockerId]["coords"]) > 2.5 then
        ...
        Bank.BigBanks[bankId]["lockers"][lockerId][state] = bool
    else
        if #(GetEntityCoords(GetPlayerPed(source)) - Bank.SmallBanks[bankId]["lockers"][lockerId]["coords"]) > 2.5 then
...
RegisterNetEvent('nmsh-bankrobbery:server:recieveItem', function(type, bankId, lockerId)
    ...
    if type == "small" then
        if #(GetEntityCoords(GetPlayerPed(src)) - Bank.SmallBanks[bankId]["lockers"][lockerId]["coords"]) > 2.5 then
        ...
        local itemType = math.random(#Bank.RewardTypes)
        ...
        -- if tier ~= 4 then
            if Bank.RewardTypes[itemType].type == "item" then
                local item = Bank.LockerRewards["tier"..tier][math.random(#Bank.LockerRewards["tier"..tier])]
                ...
                elseif Bank.RewardTypes[itemType].type == "money" then
                local info = {
            ...
            if math.random(1, 100) > House.NextRobbery.chance then
                Player.Functions.AddItem(BankFleeca.NextRobbery.item, 1)
            end
        -- else
    ...
    elseif type == "paleto" then
        if #(GetEntityCoords(GetPlayerPed(source)) - Bank.BigBanks["paleto"]["lockers"][lockerId]["coords"]) > 2.5 then
        ...
        if WeaponChance ~= odd1 then
            if tier ~= 4 then
                 if Bank.RewardTypes[itemType].type == "item" then
                    local item = Bank.LockerRewardsPaleto["tier"..tier][math.random(#Bank.LockerRewardsPaleto["tier"..tier])]
                    local itemAmount = math.random(item.minAmount, item.maxAmount)
                    ply.Functions.AddItem(item.item, itemAmount)
                    TriggerClientEvent('inventory:client:ItemBox', src, QBCore.Shared.Items[item.item], "add")
                 elseif Bank.RewardTypes[itemType].type == "money" then
    ...
    elseif type == "pacific" then
        if #(GetEntityCoords(GetPlayerPed(source)) - Bank.BigBanks["pacific"]["lockers"][lockerId]["coords"]) > 2.5 then
        ...
        if WeaponChance ~= odd1 or WeaponChance ~= odd2 then
            if tier ~= 4 then
                if Bank.RewardTypes[itemType].type == "item" then
                    local item = Bank.LockerRewardsPacific["tier"..tier][math.random(#Bank.LockerRewardsPacific["tier"..tier])]
                    local maxAmount
...
AddEventHandler('nmsh-bankrobbery:server:setTimeout', function()
    ...
    CreateThread(function()
        SetTimeout(60000 * 90, function()
            ...
            timeOut = false
            robberyBusy = false
            Bank.OnRobberyTimeoutEnd("paleto")
            Bank.OnRobberyTimeoutEnd("pacific")
        end)
    end)
end)
...
AddEventHandler('nmsh-bankrobbery:server:SetSmallBankTimeout', function(bankId)
    ...
    CreateThread(function()
        SetTimeout(60000 * 30, function()
            ...
            timeOut = false
            robberyBusy = false
            Bank.OnRobberyTimeoutEnd(bankId)
        end)
    end)
end)
...
RegisterNetEvent('nmsh-bankrobbery:server:callCops', function(type, bank, coords)
    if type == "small" then
        if not Bank.SmallBanks[bank]["alarm"] then
...
RegisterNetEvent('nmsh-bankrobbery:server:SetStationStatus', function(key, isHit)
    Bank.PowerStations[key].hit = isHit
...
RegisterNetEvent('thermite:StartServerFire', function(coords, maxChildren, isGasFire)
    ...
    local thermiteCoords = Bank.BigBanks['pacific'].thermite[1].coords
    local thermite2Coords = Bank.BigBanks['pacific'].thermite[2].coords
    local thermite3Coords = Bank.BigBanks['paleto'].thermite[1].coords
...
QBCore.Functions.CreateCallback('nmsh-bankrobbery:server:GetConfig', function(_, cb)
    cb(Bank.PowerStations, Bank.BigBanks, Bank.SmallBanks)
end)
