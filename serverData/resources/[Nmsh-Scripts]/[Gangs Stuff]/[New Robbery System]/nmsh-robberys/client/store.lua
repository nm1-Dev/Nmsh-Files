...
local qb_target = exports['nmsh-target']
local CopCount = 0

local function SendDispatch()
    if GetResourceState("nmsh-dispatch") == "started" then
        exports['nmsh-dispatch']:StoreRobbery()
    end
end
...
local function Alert(id)
    ...
    cfg.alerted = true
    TriggerServerEvent("nmsh-storerobbery:server:registerAlert", id, true)
...
local function EntityDamage(victim, culprit)
    ...
    if CStore.MinPolice > CopCount then return end
    local alerted = false
    if not CurrentStore then return end
    if not TempStoreData then return end
    local cfg = CStore.Store[CurrentStore]
    if cfg.alerted then return end
...
local function RobRegistar()
    ...
    if CStore.MinPolice > CopCount then
        QBCore.Functions.Notify("Not enough cops", "error")
        return
    end
    local ped = cache.ped
    local StoreConfig = CStore.Store[CurrentStore]
    ...
    TriggerServerEvent("nmsh-storerobbery:server:setUse", CurrentStore, RegZoneID, true)
    ...
    local prize = math.floor(math.random(CStore.Prize.min, CStore.Prize.max))
    local success = exports['nmsh-minigames']:MineSweep(prize, 10, 3, "left")
    if success then
        local itemName = CStore.Inventory == "ox" and ox_inventory:Items(CStore.Prize.item).label or
            QBCore.Shared.Items[CStore.Prize.item]?.label
        lib.callback("nmsh-houserobbery:server:getPrize", false, function(cb)
            QBCore.Functions.Notify(
                ("You got %s %s"):format(success,
                    CStore.Prize.item and itemName or "Cash"), "success")
        end, success, CurrentStore, RegZoneID)
    end
    TaskPlayAnim(ped, anim, "exit", 8.0, 8.0, -1, 0, 1.0, false, false, false)
    TriggerServerEvent("nmsh-storerobbery:server:setUse", CurrentStore, RegZoneID, false)
end
...
local function SearchCombination(storeid, sid)
    ...
    if searchLoc.iscomputer then
        ...
        local success = exports['nmsh-minigames']:OpenTerminal()
        if success then
            ...
            if canGet then
                ...
                SetTimeout(1000 * TimeToWait, function()
                    lib.callback.await("nmsh-storerobbery:server:combination", false, storeid, sid, true)
                    QBCore.Functions.Notify("You got the combination key")
                end)
            else
                lib.callback.await("nmsh-storerobbery:server:combination", false, storeid, sid, false)
        ...
        QBCore.Functions.Progressbar('search-combination', 'Searching for combination', 5000, false, true,
            { -- Name | Label | Time | useWhileDead | canCancel
                ...
                lib.callback.await("nmsh-storerobbery:server:combination", false, storeid, sid, canGet)
                if canGet then
                    QBCore.Functions.Notify("You got the combination key")
                else
...
local function SetupStore(id)
    ...
    local function UpdateConfig()
        if not lib.table.matches(cfg, CStore.Store[id]) then
            cfg = CStore.Store[id]
        end
    end
    ...
    local function Hack(self)
        ...
        if cfg.cooldown then return end
        local success = exports["memorygame"]:thermiteminigame(10, 3, 3, 10)
        TriggerServerEvent("nmsh-storerobbery:server:setHackUse", id, true)
        -- if success then
            exports["memorygame"]:thermiteminigame(10, 3, 3, 10,
            function() -- success
                TriggerServerEvent("nmsh-houserobbery:server:setHackedState", self.storeid, true)
                Wait(500)
                Alert(id)
            end,
            function() -- failure
        ...
        -- end
        TriggerServerEvent("nmsh-storerobbery:server:setHackUse", id, false)
    end
    local function OpenPinPrompt()
        ...
        if num == cfg.combination then
            lib.callback.await("nmsh-storerobbery:server:setSafeState", false, CurrentStore, true)
...
local function ResetStore(id)
    if TempStoreData.hack then
        if CStore.Target == "ox" then
            ox_target:removeZone(TempStoreData.hack)
        elseif CStore.Target == "qb" then
...
local function ConfigContext()
    ...
    local opts = {}
    for k, v in pairs(CStore.Store) do
        opts[#opts + 1] = {
...
lib.callback.register("nmsh-storerobbery:client:openConfigContext", function()
    return ConfigContext()
end)

lib.callback.register("nmsh-storerobbery:client:resetStore", function()
    if not CurrentStore then return end
    local cfg = CStore.Store[CurrentStore]
    if not cfg then return end
...
RegisterNetEvent("nmsh-storerobbery:client:setConfigs", function(cfg)
    CStore.Store = cfg
end)

RegisterNetEvent("nmsh-storerobbery:client:setStoreConfig", function(id, cfg)
    if not id or not cfg then return end
    if not type(cfg) == "table" then return end
    if not CStore.Store[id] then return end
    CStore.Store[id] = cfg
end)
    ...
    local options = {
        {
            ...
            canInteract = function(entity, distance, coords, name, bone)
                return entity and GetEntityHealth(entity) < 1000 and CurrentStore and not CStore.Store[CurrentStore]
                    .cooldown
            end,
