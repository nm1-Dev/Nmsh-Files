...
RegisterNetEvent('thermite:UseThermite', function()
    ...
    if closestStation ~= 0 then
        ...
        local dist = #(pos - Bank.PowerStations[closestStation].coords)
        if dist < 1.5 then
            if CurrentCops >= Bank.MinimumThermitePolice then
                if not Bank.PowerStations[closestStation].hit then
                    loadAnimDict("weapon@w_sp_jerrycan")
                    TaskPlayAnim(PlayerPedId(), "weapon@w_sp_jerrycan", "fire", 3.0, 3.9, 180, 49, 0, 0, 0, 0)
                    Bank.ShowRequiredItems(requiredItems, false)
                ...
                QBCore.Functions.Notify(Lang:t("error.minium_police_required", {police = Bank.MinimumThermitePolice}), "error")
            end
        end
    ...
    QBCore.Functions.TriggerCallback("thermite:server:check", function(success)
        if success then
            ...
            elseif currentGate ~= 0 then
                ...
                Bank.DoorlockAction(currentGate, false)
                currentGate = 0
            end
        end
        cb('ok')
...
CreateThread(function()
    for k = 1, #Bank.PowerStations do
        local stationZone = BoxZone:Create(Bank.PowerStations[k].coords, 1.0, 1.0, {
            name = 'powerstation_coords_'..k,
            heading = 90.0,
            minZ = Bank.PowerStations[k].coords.z - 1,
            maxZ = Bank.PowerStations[k].coords.z + 1,
            debugPoly = false
        })
        stationZone:onPlayerInOut(function(inside)
            if inside and not Bank.PowerStations[k].hit then
                closestStation = k
                Bank.ShowRequiredItems(requiredItems, true)
            else
