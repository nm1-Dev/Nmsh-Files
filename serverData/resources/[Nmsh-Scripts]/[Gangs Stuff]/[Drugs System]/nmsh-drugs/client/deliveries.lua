...
AddStateBagChangeHandler('isLoggedIn', nil, function(_, _, value)
    if value then
        QBCore.Functions.TriggerCallback('nmsh-drugs:server:RequestConfig', function(DealerConfig)
...
local function RequestDelivery()
    if not waitingDelivery then
        ...
        TriggerServerEvent('nmsh-drugs:server:giveDeliveryItems', waitingDelivery)
        SetTimeout(2000, function()
            TriggerServerEvent('nmsh-phone:server:sendNewMail', {
                ...
                button = {
                    enabled = true,
                    buttonEvent = "nmsh-drugs:client:setLocation",
...
local function DeliverStuff()
    if deliveryTimeout > 0 then
        ...
        QBCore.Functions.Progressbar("work_dropbox", Lang:t("info.delivering_products"), 3500, false, true, {
            ...
            TriggerServerEvent('nmsh-drugs:server:successDelivery', activeDelivery, true)
            activeDelivery = nil
            if Config.UseTarget then
                exports['nmsh-target']:RemoveZone('drugDeliveryZone')
            else
        ...
        TriggerServerEvent('nmsh-drugs:server:successDelivery', activeDelivery, false)
    end
    deliveryTimeout = 0
end
...
function InitZones()
    ...
    if Config.UseTarget then
        for k,v in pairs(Config.Dealers) do
            exports["nmsh-target"]:AddBoxZone("dealer_"..k, vector3(v.coords.x, v.coords.y, v.coords.z), 1.5, 1.5, {
                name = "dealer_"..k,
                heading = v.heading,
                minZ = v.coords.z - 1,
                maxZ = v.coords.z + 1,
...
RegisterNetEvent('nmsh-drugs:client:RefreshDealers', function(DealerData)
    if not Config.UseTarget and dealerCombo then dealerCombo:destroy() end
...
RegisterNetEvent('nmsh-drugs:client:updateDealerItems', function(itemData, amount)
    TriggerServerEvent('nmsh-drugs:server:updateDealerItems', itemData, amount, currentDealer)
end)

RegisterNetEvent('nmsh-drugs:client:setDealerItems', function(itemData, amount, dealer)
    Config.Dealers[dealer]["products"][itemData.slot].amount = Config.Dealers[dealer]["products"][itemData.slot].amount - amount
end)

RegisterNetEvent('nmsh-drugs:client:setLocation', function(locationData)
    ...
    SetMapBlip(activeDelivery["coords"]["x"], activeDelivery["coords"]["y"])
    if Config.UseTarget then
        exports["nmsh-target"]:AddBoxZone('drugDeliveryZone', vector3(activeDelivery["coords"].x, activeDelivery["coords"].y, activeDelivery["coords"].z), 1.5, 1.5, {
...
RegisterNetEvent('nmsh-drugs:client:sendDeliveryMail', function(type, deliveryData)
