...
local function RobberyPed()
    if Config.UseTarget then
        exports['nmsh-target']:AddEntityZone('stealingPed', stealingPed, {
            name = 'stealingPed',
            debugPoly = false,
        }, {
            options = {
                {
                    ...
                    action = function()
                        ...
                        TriggerServerEvent('nmsh-drugs:server:giveStealItems', stealData.item, stealData.amount)
                        TriggerEvent('inventory:client:ItemBox', QBCore.Shared.Items[stealData.item], "add")
                        stealingPed = nil
                        stealData = {}
                        exports['nmsh-target']:RemoveZone('stealingPed')
                    end,
...
local function SellToPed(ped)
    ...
    if hasTarget then
        while pedDist < 1.5 and not IsPedDeadOrDying(ped) do
            ...
            if getRobbed <= Config.RobberyChance then
                TriggerServerEvent('nmsh-drugs:server:robCornerDrugs', drugType, bagAmount)
                ...
                if pedDist2 < 1.5 and cornerselling then
                    if Config.UseTarget and not zoneMade then
                        zoneMade = true
                        exports['nmsh-target']:AddEntityZone('sellingPed', ped, {
                            name = 'sellingPed',
                            debugPoly = false,
                        }, {
                            options = {
                                {
                                    ...
                                    action = function(entity)
                                        if IsPedInAnyVehicle(PlayerPedId(), false) then
                                            ...
                                            exports['nmsh-target']:RemoveZone('sellingPed')
                                            return
                                        else
                                            QBCore.Functions.Progressbar("cornerSelling", Lang:t("info.selling_to_ped"), '5000', false, false, {
                                                ...
                                                TriggerServerEvent('nmsh-drugs:server:sellCornerDrugs', drugType, bagAmount, randomPrice)
                                                hasTarget = false
...
-- Events
RegisterNetEvent('nmsh-drugs:client:cornerselling', function()
    QBCore.Functions.TriggerCallback('nmsh-drugs:server:cornerselling:getAvailableDrugs', function(result)
...
RegisterNetEvent('nmsh-drugs:client:refreshAvailableDrugs', function(items)
