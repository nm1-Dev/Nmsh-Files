...
QBCore.Functions.CreateCallback('nmsh-drugs:server:cornerselling:getAvailableDrugs', function(source, cb)
    cb(getAvailableDrugs(source))
end)

RegisterNetEvent('nmsh-drugs:server:giveStealItems', function(drugType, amount)
    local Player = QBCore.Functions.GetPlayer(source)
...
RegisterNetEvent('nmsh-drugs:server:sellCornerDrugs', function(drugType, amount, price)
    local src = source
    ...
    if hasItem.amount >= amount then
        ...
        TriggerClientEvent('nmsh-drugs:client:refreshAvailableDrugs', src, getAvailableDrugs(src))
    else
        TriggerClientEvent('nmsh-drugs:client:cornerselling', src)
    end
end)

RegisterNetEvent('nmsh-drugs:server:robCornerDrugs', function(drugType, amount)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    local availableDrugs = getAvailableDrugs(src)
