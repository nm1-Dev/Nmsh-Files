...
local function ProcessChemicals()
	...
	QBCore.Functions.Progressbar("search_register", Lang:t("progressbar.processing"), 15000, false, true, {
		...
		TriggerServerEvent('nmsh-drugprocessing:processChemicals')
...
local function ProcessTempUp()
	...
	QBCore.Functions.Progressbar("search_register", Lang:t("progressbar.temp_up"), 15000, false, true, {
		...
		TriggerServerEvent('nmsh-drugprocessing:processTempUp')
...
local function ProcessTempDown()
	...
	QBCore.Functions.Progressbar("search_register", Lang:t("progressbar.temp_down"), 15000, false, true, {
		...
		TriggerServerEvent('nmsh-drugprocessing:processTempDown')
...
local function ProcessProduct()
	...
	QBCore.Functions.Progressbar("search_register", Lang:t("progressbar.packing"), 15000, false, true, {
		...
		TriggerServerEvent('nmsh-drugprocessing:processMeth')
...
RegisterNetEvent('nmsh-drugprocessing:ProcessChemicals', function()
...
RegisterNetEvent('nmsh-drugprocessing:ChangeTemp', function()
			...
			if result.ret then
				exports['nmsh-ui']:Thermite(function(success)
					if success then
						...
						TriggerServerEvent('nmsh-drugprocessing:cancelProcessing')
						TriggerServerEvent('nmsh-drugprocessing:processFailUp')
					end
				end, 10, 5, 3)
			else
				QBCore.Functions.Notify(Lang:t("error.no_item", {item = result.item}))
...
RegisterNetEvent('nmsh-drugprocessing:ChangeTemp2', function()
			...
			if result.ret then
				exports['nmsh-ui']:Thermite(function(success)
					if success then
						...
						TriggerServerEvent('nmsh-drugprocessing:cancelProcessing')
						TriggerServerEvent('nmsh-drugprocessing:processFailDown')
					end
				end, 10, 5, 3)
			else
				QBCore.Functions.Notify(Lang:t("error.no_item", {item = result.item}))
...
RegisterNetEvent('nmsh-drugprocessing:ProcessProduct', function()
	local coords = GetEntityCoords(PlayerPedId(source))
...
RegisterNetEvent('nmsh-drugprocessing:EnterLab', function()
	local ped = PlayerPedId()
...
RegisterNetEvent('nmsh-drugprocessing:ExitLab', function()
	local ped = PlayerPedId()
	local pos = GetEntityCoords(ped)
