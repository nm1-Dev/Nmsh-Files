...
local function ProcessHeroin()
	...
	QBCore.Functions.Progressbar("search_register", Lang:t("progressbar.processing"), 15000, false, true, {
		...
		TriggerServerEvent('nmsh-drugprocessing:processPoppyResin')
...
RegisterNetEvent('nmsh-drugprocessing:ProcessPoppy', function()
...
RegisterNetEvent("nmsh-drugprocessing:processHeroin",function()
	QBCore.Functions.TriggerCallback('nmsh-drugprocessing:validate_items', function(result)
		if result.ret then
			ProcessHeroin()
		else
...
RegisterNetEvent("nmsh-drugprocessing:pickHeroin", function()
	local playerPed = PlayerPedId()
	...
	if nearbyObject and IsPedOnFoot(playerPed) then
		...
		QBCore.Functions.Progressbar("search_register", Lang:t("progressbar.collecting"), 10000, false, true, {
			...
			TriggerServerEvent('nmsh-drugprocessing:pickedUpPoppy')
			isPickingUp = false
