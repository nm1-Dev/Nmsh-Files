...
local function createChemicalMenu()
    local chemMenu = {
        ...
        },
        {
			...
			params = {
                event = "nmsh-drugprocessing:hydrochloric_acid",
            }
        },
        {
            ...
            txt = Lang:t("menu.chemicals"),
			params = {
                event = "nmsh-drugprocessing:sodium_hydroxide",
            }
        },
        {
            ...
            txt = Lang:t("menu.chemicals"),
			params = {
                event = "nmsh-drugprocessing:sulfuric_acid",
            }
        },
        {
        ...
        },
        {
			...
			params = {
                event = exports['nmsh-menu']:closeMenu(),
            }
        },
    }
    exports['nmsh-menu']:openMenu(chemMenu)
end
RegisterNetEvent('nmsh-drugprocessing:chemicalmenu', createChemicalMenu)
...
local function process_hydrochloric_acid()
	...
	QBCore.Functions.Progressbar("search_register", Lang:t("progressbar.processing"), 15000, false, true, {
		...
		TriggerServerEvent('nmsh-drugprocessing:processHydrochloric_acid')
		...
		while timeLeft > 0 do
			...
			if #(GetEntityCoords(playerPed)-Config.CircleZones.ChemicalsConvertionMenu.coords) > 4 then
				TriggerServerEvent('nmsh-drugprocessing:cancelProcessing')
				break
			end
		end
...
local function process_lsa()
	...
	QBCore.Functions.Progressbar("search_register", Lang:t("progressbar.processing"), 15000, false, true, {
		...
		TriggerServerEvent('nmsh-drugprocessing:process_lsa')
...
local function process_sulfuric_acid()
	...
	QBCore.Functions.Progressbar("search_register", Lang:t("progressbar.processing"), 15000, false, true, {
		...
		TriggerServerEvent('nmsh-drugprocessing:processprocess_sulfuric_acid')
...
local function process_sodium_hydroxide()
	...
	QBCore.Functions.Progressbar("search_register", Lang:t("progressbar.processing"), 15000, false, true, {
		...
		TriggerServerEvent('nmsh-drugprocessing:processsodium_hydroxide')
...
RegisterNetEvent("nmsh-drugprocessing:hydrochloric_acid", function()
    QBCore.Functions.TriggerCallback('nmsh-drugprocessing:validate_items', function(result)
		if result then
...
RegisterNetEvent("nmsh-drugprocessing:lsa", function()
    QBCore.Functions.TriggerCallback('nmsh-drugprocessing:validate_items', function(result)
		if result then
			process_lsa()
		else
...
RegisterNetEvent("nmsh-drugprocessing:sulfuric_acid", function()
    QBCore.Functions.TriggerCallback('nmsh-drugprocessing:validate_items', function(result)
		if result then
...
RegisterNetEvent("nmsh-drugprocessing:sodium_hydroxide", function()
    QBCore.Functions.TriggerCallback('nmsh-drugprocessing:validate_items', function(result)
		if result then
...
RegisterNetEvent("nmsh-drugprocessing:pickChemicals", function()
	local playerPed = PlayerPedId()
	...
	if nearbyObject and IsPedOnFoot(playerPed) then
		...
		QBCore.Functions.Progressbar("search_register", Lang:t("progressbar.pickup_chemicals"), 10000, false, true, {
			...
			TriggerServerEvent('nmsh-drugprocessing:pickedUpChemicals')
			isPickingUp = false
