...
local function RollJoint()
	...
	QBCore.Functions.Progressbar("search_register", Lang:t("progressbar.rolling_joint"), 15000, false, true, {
		...
		TriggerServerEvent('nmsh-drugprocessing:rollJoint')
...
local function BagSkunk()
	...
	QBCore.Functions.Progressbar("search_register", Lang:t("progressbar.bagging_skunk"), 15000, false, true, {
		...
		TriggerServerEvent('nmsh-drugprocessing:bagskunk')
		local timeLeft = Config.Delays.WeedProcessing / 1000
		while timeLeft > 0 do
			Wait(1000)
			timeLeft -= 1
...
local function ProcessWeed()
	...
	QBCore.Functions.Progressbar("search_register", Lang:t("progressbar.processing"), 15000, false, true, {
		...
		TriggerServerEvent('nmsh-drugprocessing:processCannabis')
		local timeLeft = Config.Delays.WeedProcessing / 1000
		while timeLeft > 0 do
			Wait(1000)
			timeLeft -= 1
...
RegisterNetEvent("nmsh-drugprocessing:processWeed",function()
...
RegisterNetEvent('nmsh-drugprocessing:EnterWWarehouse', function()
	local ped = PlayerPedId()
...
RegisterNetEvent('nmsh-drugprocessing:ExitWWarehouse', function()
	local ped = PlayerPedId()
	local pos = GetEntityCoords(ped)
...
RegisterNetEvent("nmsh-drugprocessing:pickWeed", function()
	local playerPed = PlayerPedId()
	...
	if nearbyObject and IsPedOnFoot(playerPed) then
		if not isPickingUp then
			...
			QBCore.Functions.Progressbar("search_register", Lang:t("progressbar.collecting"), 10000, false, true, {
				...
				spawnedWeeds -= 1
				TriggerServerEvent('nmsh-drugprocessing:pickedUpCannabis')
				isPickingUp = false
			end, function()
				ClearPedTasks(PlayerPedId())
				isPickingUp = false
...
RegisterNetEvent('nmsh-drugprocessing:client:rollJoint', function()
    QBCore.Functions.TriggerCallback('nmsh-drugprocessing:validate_items', function(result)
		if result.ret then
			RollJoint()
		else
...
RegisterNetEvent('nmsh-drugprocessing:client:bagskunk', function()
    QBCore.Functions.TriggerCallback('nmsh-drugprocessing:validate_items', function(result)
		if result.ret then
			BagSkunk()
		else
