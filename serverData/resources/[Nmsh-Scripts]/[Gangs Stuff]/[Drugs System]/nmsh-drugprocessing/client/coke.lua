...
local function ProcessCoke()
	...
	QBCore.Functions.Progressbar("search_register", Lang:t("progressbar.processing"), 15000, false, true, {
		...
		TriggerServerEvent('nmsh-drugprocessing:processCocaLeaf')
...
local function CutCokePowder()
	...
	QBCore.Functions.Progressbar("search_register", Lang:t("progressbar.processing"), 15000, false, true, {
		...
		TriggerServerEvent('nmsh-drugprocessing:processCocaPowder')
...
local function ProcessBricks()
	...
	QBCore.Functions.Progressbar("search_register", Lang:t("progressbar.packing"), 15000, false, true, {
		...
		TriggerServerEvent('nmsh-drugprocessing:processCocaBrick')
...
RegisterNetEvent('nmsh-drugprocessing:ProcessCocaFarm', function()
...
RegisterNetEvent('nmsh-drugprocessing:ProcessCocaPowder', function()
	local coords = GetEntityCoords(PlayerPedId())
	local amount = 10
	local amount2 = 5
...
RegisterNetEvent('nmsh-drugprocessing:ProcessBricks', function()
...
RegisterNetEvent('nmsh-drugprocessing:EnterCWarehouse', function()
	local ped = PlayerPedId()
...
RegisterNetEvent('nmsh-drugprocessing:ExitCWarehouse', function()
	local ped = PlayerPedId()
	local pos = GetEntityCoords(ped)
...
RegisterNetEvent('nmsh-drugprocessing:pickCocaLeaves', function()
	local playerPed = PlayerPedId()
	...
	if nearbyObject and IsPedOnFoot(playerPed) then
		if not isPickingUp then
			...
			QBCore.Functions.Progressbar("search_register", Lang:t("progressbar.collecting"), 10000, false, true, {
				...
				TriggerServerEvent('nmsh-drugprocessing:pickedUpCocaLeaf')
				isPickingUp = false
			end, function()
				ClearPedTasks(playerPed)
				isPickingUp = false
