...
RegisterNetEvent('nmsh-weedplanting:server:ClearPlant', function(netId)
...
RegisterNetEvent('nmsh-weedplanting:server:HarvestPlant', function(netId)
    local entity = NetworkGetEntityFromNetworkId(netId)
    if not WeedPlants[entity] then return end
    local src = source
...
RegisterNetEvent('nmsh-weedplanting:server:PoliceDestroy', function(netId)
    local entity = NetworkGetEntityFromNetworkId(netId)
    if not WeedPlants[entity] then return end
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    ...
    if DoesEntityExist(entity) then
        ...
        TriggerClientEvent('nmsh-weedplanting:client:FireGoBrrrrrrr', -1, WeedPlants[entity].coords)
...
RegisterNetEvent('nmsh-weedplanting:server:GiveWater', function(netId)
    local entity = NetworkGetEntityFromNetworkId(netId)
    if not WeedPlants[entity] then return end
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
RegisterNetEvent('nmsh-weedplanting:server:GiveFertilizer', function(netId)
    local entity = NetworkGetEntityFromNetworkId(netId)
    if not WeedPlants[entity] then return end
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
RegisterNetEvent('nmsh-weedplanting:server:AddMaleSeed', function(netId)
    local entity = NetworkGetEntityFromNetworkId(netId)
    if not WeedPlants[entity] then return end
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
RegisterNetEvent('nmsh-weedplanting:server:CreateNewPlant', function(coords)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if not Player then return end
...
RegisterNetEvent('nmsh-weedplanting:server:GetFullWateringCan', function(netId)
...
QBCore.Functions.CreateCallback('nmsh-weedplanting:server:GetPlantData', function(source, cb, netId)
    local entity = NetworkGetEntityFromNetworkId(netId)
...
QBCore.Functions.CreateUseableItem(Shared.FemaleSeed, function(source)
    ...
    if tub ~= nil then
        TriggerClientEvent("nmsh-weedplanting:client:UseWeedSeed", source)
    else
...
QBCore.Functions.CreateUseableItem(Shared.EmptyCanItem, function(source)
    local src = source
    TriggerClientEvent("nmsh-weedplanting:client:OpenFillWaterMenu", src)
end)
