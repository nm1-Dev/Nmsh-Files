...
RegisterNetEvent('nmsh-weedplanting:server:CollectPackageGoods', function()
    local src = source
    ...
    elseif packageCache[citizenid] == 'done' then
        ...
        TriggerClientEvent('nmsh-weedplanting:client:PackageGoodsReceived', src)
...
RegisterNetEvent('nmsh-weedplanting:server:DestroyWaitForPackage', function()
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if not Player then return end
    local citizenid = Player.PlayerData.citizenid
...
RegisterNetEvent('nmsh-weedplanting:server:WeedrunDelivery', function()
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if not Player then return end
...
QBCore.Functions.CreateCallback('nmsh-weedplanting:server:PackageGoods', function(source, cb)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    local citizenid = Player.PlayerData.citizenid
