...
local createDropOffPed = function(coords)
	...
	exports['nmsh-target']:AddTargetEntity(deliveryPed, {
		options = {
			{
				type = 'client',
				event = 'nmsh-weedplanting:client:DeliverWeed',
...
local createNewDropOff = function()
    ...
    hasDropOff = true
    TriggerEvent('nmsh-phone:client:CustomNotification', _U('weedrun_delivery_title'), _U('weedrun_delivery_godropoff'), 'fas fa-cannabis', '#00FF00', 8000)
	...
	dropOffArea:onPlayerInOut(function(isPointInside, point)
		if isPointInside then
			if not deliveryPed then
				TriggerEvent('nmsh-phone:client:CustomNotification', _U('weedrun_delivery_title'), _U('weedrun_delivery_makedropoff'), 'fas fa-cannabis', '#00FF00', 8000)
...
RegisterNetEvent('nmsh-weedplanting:client:StartPackage', function(data)
    ...
    QBCore.Functions.Progressbar('weedrun_pack', _U('handing_over_weed'), 4000, false, true, {
        ...
        QBCore.Functions.TriggerCallback('nmsh-weedplanting:server:PackageGoods', function(result)
            ...
            packageZone:onPlayerInOut(function(isPointInside, point)
                if not isPointInside then
                    ...
                    if waitingForPackage then
                        TriggerServerEvent('nmsh-weedplanting:server:DestroyWaitForPackage')
...
RegisterNetEvent('nmsh-weedplanting:client:PackageGoodsReceived', function()
    if waitingForPackage then
        packageZone:destroy()
        waitingForPackage = false
    end
...
RegisterNetEvent('nmsh-weedplanting:client:ClockIn', function()
    if delivering then return end
    delivering = true
    TriggerEvent('nmsh-phone:client:CustomNotification', _U('weedrun_delivery_title'), _U('weedrun_delivery_waitfornew'), 'fas fa-cannabis', '#00FF00', 8000)
    Wait(math.random(Shared.DeliveryWaitTime[1], Shared.DeliveryWaitTime[2]))
    createNewDropOff()
end)

RegisterNetEvent('nmsh-weedplanting:client:ClockOut', function()
...
RegisterNetEvent('nmsh-weedplanting:client:DeliverWeed', function()
	...
	if #(GetEntityCoords(ped) - GetEntityCoords(deliveryPed)) < 5.0 then
		madeDeal = true
		exports['nmsh-target']:RemoveTargetEntity(deliveryPed)

		-- Alert Cops
		if math.random(100) <= Shared.CallCopsChance then
            if GetResourceState(Shared.Dispatch) == 'started' then
                exports[Shared.Dispatch]:DrugSale() -- Project-SLoth nmsh-dispatch
            end
        end
		...
		Wait(1000)
		TriggerServerEvent('nmsh-weedplanting:server:WeedrunDelivery')
		...
		dropOffArea:destroy()
		Wait(2000)
		TriggerEvent('nmsh-phone:client:CustomNotification', _U('weedrun_delivery_title'), _U('weedrun_delivery_success'), 'fas fa-cannabis', '#00FF00', 20000)
        ClearPedTasks(ped)
...
CreateThread(function()
    ...
    exports['nmsh-target']:AddBoxZone('weed-run-zone', vector3(Shared.WeedRunStart.x, Shared.WeedRunStart.y, Shared.WeedRunStart.z), 0.8, 0.8, {
        name = 'weed-run-zone',
        ...
        options = {
            { -- Create Package
                type = 'client',
                event = 'nmsh-weedplanting:client:StartPackage',
            ...
            { -- Receive Package
                type = 'server',
                event = 'nmsh-weedplanting:server:CollectPackageGoods',
                icon = 'fas fa-circle-chevron-right',
            ...
            { -- Clock In for deliveries
                type = 'client',
                event = 'nmsh-weedplanting:client:ClockIn',
            ...
            { -- Clock out for deliveries
                type = 'client',
                event = 'nmsh-weedplanting:client:ClockOut',
                icon = 'fas fa-stopwatch',
