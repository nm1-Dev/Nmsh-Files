...
local function RegisterTraphouseEntranceTarget(traphouseID, traphouseData)
    ...
    exports['nmsh-target']:AddBoxZone(boxName, coords, boxData.length, boxData.width, {
        name = boxName,
        ...
        options = {
            {
                type = 'client',
                event = 'nmsh-traphouse:client:EnterTraphouse',
...
local function RegisterTraphouseInteractionZone(traphouseID, traphouseData)
    ...
    zone:onPlayerInOut(function (isPointInside)
        if isPointInside then
            ...
            exports['qb-core']:HideText()
            TriggerEvent('nmsh-traphouse:client:target:CloseMenu')
        end
...
local function RegisterTraphouseInteractionTarget(traphouseID, traphouseData)
    ...
    local options = {
        {
            type = "client",
            event = "nmsh-traphouse:client:target:TakeOver",
    ...
    if IsKeyHolder then
        options = {
            {
                type = "client",
                event = "nmsh-traphouse:client:target:ViewInventory",
                label = Lang:t("targetInfo.inventory"),
                traphouseData = traphouseData
            },
            {
                type = "client",
                event = "nmsh-traphouse:client:target:TakeMoney",
        ...
        if IsHouseOwner then
            options[#options+1] = {
                type = "client",
                event = "nmsh-traphouse:client:target:SeePinCode",
...
local function RegisterTraphouseExitTarget(coords, traphouseID, traphouseData)
    ...
    exports['nmsh-target']:AddBoxZone(boxName, coords, boxData.length, boxData.width, {
        ...
        options = {
            {
                type = "client",
                event = "nmsh-traphouse:client:target:ExitTraphouse",
...
local function OpenHeaderMenu(data)
    ...
    headerMenu[#headerMenu+1] = {
        ...
        params = {
            event = "nmsh-traphouse:client:target:CloseMenu",
        }
    }
...
local function LeaveTraphouse(k, data)
    ...
    DoScreenFadeOut(250)
    Wait(250)
    exports['nmsh-interior']:DespawnInterior(TraphouseObj, function()
    ...
    if Config.UseTarget then
        exports['nmsh-target']:RemoveZone('traphouseInteraction' .. k)
        data.polyzoneBoxData['interaction'].created = false

        exports['nmsh-target']:RemoveZone('traphouseExit' .. k)
...
RegisterNetEvent('nmsh-traphouse:client:EnterTraphouse', function()
    if ClosestTraphouse ~= nil then
...
RegisterNetEvent('nmsh-traphouse:client:TakeoverHouse', function(TraphouseId)
    QBCore.Functions.Progressbar("takeover_traphouse", Lang:t("info.taking_over"), math.random(1000, 3000), false, true, {
        ...
        TriggerServerEvent('nmsh-traphouse:server:AddHouseKeyHolder', PlayerData.citizenid, TraphouseId, true)
    end, function()
        QBCore.Functions.Notify(Lang:t("error.cancelled"), "error")
    end)
end)

RegisterNetEvent('nmsh-traphouse:client:target:ViewInventory', function (data)
...
RegisterNetEvent('nmsh-traphouse:client:target:TakeOver', function ()
    TriggerServerEvent('nmsh-traphouse:server:TakeoverHouse', CurrentTraphouse)
end)

RegisterNetEvent('nmsh-traphouse:client:target:TakeMoney', function ()
    TriggerServerEvent("nmsh-traphouse:server:TakeMoney", CurrentTraphouse)
end)

RegisterNetEvent('nmsh-traphouse:client:target:SeePinCode', function (data)
    QBCore.Functions.Notify(Lang:t('info.pin_code', { value = data.traphouseData.pincode }))
end)

RegisterNetEvent('nmsh-traphouse:client:target:ExitTraphouse', function (data)
    LeaveTraphouse(data.traphouseID, Config.TrapHouses[data.traphouseID])
end)

RegisterNetEvent('nmsh-traphouse:client:SyncData', function(k, data)
...
RegisterNetEvent('nmsh-traphouse:client:target:CloseMenu', function ()
                                    ...
                                    if not IsEntityDead(targetPed) then
                                        if CanRob then
                                            if InDistance then
                                                ...
                                                TriggerServerEvent('nmsh-traphouse:server:RobNpc', ClosestTraphouse)
                                                CanRob = false
                                            end
                                        end
                                    end
            ...
            if not InsideTraphouse then
                if isInsideEntranceTarget then
                    ...
                    if IsControlJustPressed(0, 38) then
                        TriggerEvent("nmsh-traphouse:client:EnterTraphouse")
                        exports['qb-core']:HideText()
                    end
                end
            else
