...
local function exploitBan(id, reason)
    ...
    TriggerEvent('nmsh-log:server:CreateLog', 'pawnshop', 'Player Banned', 'red',
        string.format('%s was banned by %s for %s', GetPlayerName(id), 'nmsh-pawnshop', reason), true)
    DropPlayer(id, 'You were permanently banned by the server for: Exploiting')
end

RegisterNetEvent('nmsh-pawnshop:server:sellPawnItems', function(itemName, itemAmount, itemPrice)
    local src = source
    ...
    TriggerClientEvent('nmsh-pawnshop:client:openMenu', src)
end)

RegisterNetEvent('nmsh-pawnshop:server:meltItemRemove', function(itemName, itemAmount, item)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if Player.Functions.RemoveItem(itemName, itemAmount) then
        TriggerClientEvent('inventory:client:ItemBox', src, QBCore.Shared.Items[itemName], 'remove')
        local meltTime = (tonumber(itemAmount) * item.time)
        TriggerClientEvent('nmsh-pawnshop:client:startMelting', src, item, tonumber(itemAmount), (meltTime * 60000 / 1000))
...
RegisterNetEvent('nmsh-pawnshop:server:pickupMelted', function(item)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    local playerCoords = GetEntityCoords(GetPlayerPed(src))
    local dist
    ...
    TriggerClientEvent('nmsh-pawnshop:client:resetPickup', src)
    TriggerClientEvent('nmsh-pawnshop:client:openMenu', src)
end)

QBCore.Functions.CreateCallback('nmsh-pawnshop:server:getInv', function(source, cb)
