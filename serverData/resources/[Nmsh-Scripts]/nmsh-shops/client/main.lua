...
local function openShop(shop, data)
    QBCore.Functions.TriggerCallback('nmsh-shops:server:SetShopInv', function(shopInvJson)
        ...
        while listen do
            if IsControlJustPressed(0, 38) then -- E
                ...
                if inChips then
                    ...
                    TriggerServerEvent("nmsh-shops:server:sellChips")
                else
                    exports["qb-core"]:KeyPressed()
...
local function createPeds()
    ...
    for k, v in pairs(Config.Locations) do
        ...
        if Config.UseTarget then
            exports['nmsh-target']:AddTargetEntity(ShopPed[k], {
                options = {
                    {
                        label = v["targetLabel"],
                        icon = v["targetIcon"],
    ...
    if Config.UseTarget then
        exports['nmsh-target']:AddTargetEntity(ShopPed["casino"], {
            options = {
                {
                    label = 'Sell Chips',
...
-- Events
RegisterNetEvent("nmsh-shops:client:UpdateShop", function(shop, itemData, amount)
    TriggerServerEvent("nmsh-shops:server:UpdateShopItems", shop, itemData, amount)
end)

RegisterNetEvent("nmsh-shops:client:SetShopItems", function(shop, shopProducts)
    Config.Locations[shop]["products"] = shopProducts
end)

RegisterNetEvent("nmsh-shops:client:RestockShopItems", function(shop, amount)
