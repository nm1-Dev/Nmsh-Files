...
RegisterNetEvent('nmsh-parkingmeter:client:payParking', function()
    ...
    QBCore.Functions.Progressbar("drilling", "Making payment...", 3000, false, true, {
        ...
        QBCore.Functions.TriggerCallback('nmsh-parkingmeter:server:checkParking', function(isPaid)
            if isPaid then
                QBCore.Functions.Notify('Parking is already paid!', 'error', 5000)
            else
                TriggerServerEvent('nmsh-parkingmeter:server:payParking', pos)
            end
        end, pos)
    end, function()
        TriggerEvent('animations:client:EmoteCommandStop')
...
RegisterNetEvent('nmsh-parkingmeter:client:checkParking', function()
    local pos = GetEntityCoords(PlayerPedId())
    TriggerEvent('animations:client:EmoteCommandStart', {"parkingmeter"})
