...
function Pet:spawnPet(source, model, item)
    ...
    TriggerClientEvent('nmsh-companion:client:callCompanion', source, model, owner, item)
end

RegisterNetEvent('nmsh-companion:server:despwan_not_owned_pet', function(hash)
...
function Pet:despawnPet(source, item, revive)
    ...
    TriggerClientEvent('nmsh-companion:client:despawn', source, item, revive)
end
...
function Pet:save_all_info(source, hash)
    ...
    if petData.info.health > 100 then
        ...
        TriggerClientEvent('nmsh-companion:client:forceKill', source, hash, 'hunger')
    end
...
RegisterNetEvent('nmsh-companion:server:setAsDespawned', function(item)
...
local function remove_item(src, Player, name, amount)
    ...
    if res then
        TriggerClientEvent('nmsh-inventory:client:ItemBox', src, QBCore.Shared.Items[name], "remove")
    end
    return res
end
...
QBCore.Functions.CreateUseableItem(core_items.food.item_name, function(source, item)
    local Player = QBCore.Functions.GetPlayer(source)
    if Player == nil then return end
    TriggerClientEvent('nmsh-companion:client:start_feeding_animation', source)
end)

RegisterNetEvent('nmsh-companion:server:increaseFood', function(item)
    local Player = QBCore.Functions.GetPlayer(source)
...
QBCore.Functions.CreateUseableItem(core_items.collar.item_name, function(source, item)
    TriggerClientEvent('nmsh-companion:client:collar_process', source)
end)
...
QBCore.Functions.CreateUseableItem(core_items.nametag.item_name, function(source, item)
    local Player = QBCore.Functions.GetPlayer(source)
    if Player == nil or item == nil then return end
    TriggerClientEvent('nmsh-companion:client:rename_name_tag', source, item)
end)

RegisterNetEvent('nmsh-companion:server:rename_name_tag', function(name)
    local Player = QBCore.Functions.GetPlayer(source)
    if Player == nil then return end
    ...
    TriggerClientEvent("nmsh-companion:client:rename_name_tagAction", source, name)
end)
...
QBCore.Functions.CreateUseableItem(core_items.groomingkit.item_name, function(source, item)
    local Player = QBCore.Functions.GetPlayer(source)
    if Player == nil then return end
    TriggerClientEvent('nmsh-companion:client:start_grooming_process', source)
end)

RegisterNetEvent('nmsh-companion:server:grooming_process', function(item)
    ...
    TriggerClientEvent('nmsh-companion:client:initialization_process', source, item, information)
end)
...
QBCore.Functions.CreateUseableItem(core_items.waterbottle.item_name, function(source, item)
    ...
    TriggerClientEvent('nmsh-companion:client:filling_animation', source, item)
end)

RegisterNetEvent('nmsh-companion:server:filling_event', function(item)
    fillwater_bottle(source, item)
end)

QBCore.Functions.CreateCallback('nmsh-companion:server:decrease_thirst', function(source, cb, data)
    local player = QBCore.Functions.GetPlayer(source)
    ...
    if pet_water_bottle.info == nil then
        ...
        print('issue with nill info: https://github.com/swkeep/nmsh-companion/issues/25')
        return
    end
    ...
    if pet_water_bottle.info.liter == nil then
        ...
        print('developer: issue with nill info -> liter: https://github.com/swkeep/nmsh-companion/issues/25')
        return
    end
...
RegisterNetEvent('nmsh-companion:server:revivePet', function(item, process_type)
    local src = source
    ...
    if process_type and process_type == 'Heal' then
        ...
        TriggerClientEvent('nmsh-companion:client:update_health_value', src, item, petData.info.health)
...
QBCore.Functions.CreateCallback('nmsh-companion:server:search_inventory', function(source, cb, cid)
...
QBCore.Functions.CreateCallback('nmsh-companion:server:search_vehicle', function(source, cb, data)
...
RegisterNetEvent('nmsh-companion:server:updateAllowedInfo', function(item, data)
...
QBCore.Functions.CreateCallback('nmsh-companion:server:renamePet', function(source, cb, item)
    local player = QBCore.Functions.GetPlayer(source)
    local current_pet_data = Pet:findbyhash(source, item.hash)
...
QBCore.Functions.CreateCallback('nmsh-companion:server:updatePedData', function(source, cb, clientRes)
    local player = QBCore.Functions.GetPlayer(source)
    if player == nil then
        cb(false)
        return
...
RegisterNetEvent('nmsh-companion:server:onPlayerUnload', function(items)
...
QBCore.Commands.Add('renamePet', 'rename pet', { { "name", "new pet name" } }, false, function(source, args)
    TriggerClientEvent("nmsh-companion:client:rename_name_tag", source, args[1])
end, 'admin')
...
RegisterNetEvent('nmsh-companion:server:ForceRemoveNetEntity', function(netId)
