...
function ActivePed:remove(index)
    ...
    TriggerServerEvent('nmsh-companion:server:ForceRemoveNetEntity', netId)
    self.data[index] = nil
...
function ActivePed:removeAll()
    ...
    for key, value in pairs(ActivePed:petsList()) do
        ...
        TriggerServerEvent('nmsh-companion:server:updateAllowedInfo', currentItem, {
            key = 'XP',
        })
    end
    TriggerServerEvent('nmsh-companion:server:onPlayerUnload', tmpHash)
    self.data = {}
    self.onControl = -1
end
...
RegisterNetEvent('nmsh-companion:client:callCompanion')
AddEventHandler('nmsh-companion:client:callCompanion', function(modelName, hostileTowardPlayer, item)
    ...
    CoreName.Functions.Progressbar("callCompanion", "Calling companion", Config.Settings.callCompanionDuration * 1000,
        ...
        QBCore.Functions.TriggerCallback('nmsh-companion:server:updatePedData', function(result)
            ...
            if petData.hostile == true then
                TriggerServerEvent('nmsh-companion:server:despwan_not_owned_pet', petData.itemData.info.hash)
                return
            end
...
function request_healing_process(ped, item, process_type)
    ...
    CoreName.Functions.Progressbar("reviveing", "Reviveing",
        ...
        TriggerServerEvent('nmsh-companion:server:revivePet', current_pet, process_type)
        TaskFollowTargetedPlayer(ped, plyID, false)
    end)
end

RegisterNetEvent('nmsh-companion:client:update_health_value', function(item, amount)
    ...
    CreateThread(function()
        ...
        while DoesEntityExist(ped) and fninished == false do
            ...
            if IsPedDeadOrDying(savedData.entity) == false and savedData.maxHealth ~= currentHealth and
                ...
                currentHealth then
                -- ped is still alive
                TriggerServerEvent('nmsh-companion:server:updateAllowedInfo', {
...
RegisterNetEvent('nmsh-companion:client:forceKill', function(hash, reason)
...
RegisterNetEvent('nmsh-companion:client:despawn')
AddEventHandler('nmsh-companion:client:despawn', function(item, revive)
    if revive ~= nil and revive == true then
        ...
        TriggerServerEvent('nmsh-companion:server:setAsDespawned', item)
        return
    end
    local plyPed = PlayerPedId()
...
RegisterNetEvent('nmsh-companion:client:start_feeding_animation', function()
    ...
    CoreName.Functions.Progressbar("feeding", "Feeding", Config.core_items.food.settings.duration * 1000, false, false,
        ...
        TriggerServerEvent('nmsh-companion:server:increaseFood', current_pet.itemData)
    end)
end)

RegisterNetEvent('nmsh-companion:client:', function()
...
function start_drinking_animation()
    ...
    CoreName.Functions.Progressbar("pet_drinking", "drinking", Config.core_items.waterbottle.settings.duration * 1000,
        ...
        QBCore.Functions.TriggerCallback('nmsh-companion:server:decrease_thirst', function(result)

        end, current_pet.itemData)
    end)
end

RegisterNetEvent('nmsh-companion:client:filling_animation', function(item)
    CoreName.Functions.Progressbar("filling_animation", "filling bottle",
        ...
        TriggerServerEvent('nmsh-companion:server:filling_event', item)
    end)
end)

RegisterNetEvent('nmsh-companion:client:rename_name_tag', function(item)
    if ActivePed:read() == nil then
        QBCore.Functions.Notify(Lang:t('error.no_pet_under_control'), 'error', 5000)
        return
    end

    local name = exports['nmsh-input']:ShowInput({
    ...
    if name then
        ...
        TriggerServerEvent('nmsh-companion:server:rename_name_tag', name.pet_name)
    end
end)



RegisterNetEvent('nmsh-companion:client:rename_name_tagAction', function(name)
    ...
    CoreName.Functions.Progressbar("waitingForName", "waiting for Name",
        ...
        QBCore.Functions.TriggerCallback('nmsh-companion:server:renamePet', function(result)
...
RegisterNetEvent('nmsh-companion:client:collar_process', function()
    ...
    if inputData then
        ...
        CoreName.Functions.Progressbar("waitingForOwenership", "waiting for new owner",
            ...
            function()
                ...
                QBCore.Functions.TriggerCallback('nmsh-companion:server:collar_change_owenership', function(result)
