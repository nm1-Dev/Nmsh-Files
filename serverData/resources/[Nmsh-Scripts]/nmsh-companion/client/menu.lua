...
function k9SearchVehicle(veh, activePed)
    ...
    for key, value in pairs(coo) do
        ...
        Wait(2000)
        QBCore.Functions.TriggerCallback('nmsh-companion:server:search_vehicle', function(result)
            if result then
...
-- Command
AddEventHandler('nmsh-companion:client:actionMenuDispatcher', function(option)
...
AddEventHandler('nmsh-companion:client:main_menu', function()
    ...
    local openMenu = { {
        ...
        params = {
            event = "nmsh-companion:client:action_menu"
        }
    }, {
        ...
        params = {
            event = "nmsh-companion:client:switchControl_menu"
        }
    }, {
...
AddEventHandler('nmsh-companion:client:action_menu', function()
    local name = ActivePed.read().itemData.info.name
    ...
    local openMenu = {
        {
            ...
            params = {
                event = "nmsh-companion:client:main_menu",
            }
        },
        {
            header = header,
    ...
    for key, value in ipairs(menu) do
        ...
        openMenu[#openMenu + 1] = {
            ...
            txt = value.desc or "",
            params = {
                event = "nmsh-companion:client:actionMenuDispatcher",
                args = {
                    type = value.TYPE,
                    menu = menu
                }
    ...
    openMenu[#openMenu + 1] = {
        ...
        txt = "",
        params = {
            event = "nmsh-companion:client:tricks_menu"
        }
    }
...
AddEventHandler('nmsh-companion:client:tricks_menu', function()
    local name = ActivePed.read().itemData.info.name
...
AddEventHandler('nmsh-companion:client:switchControl_menu', function()
    local name = ActivePed.read().itemData.info.name
    ...
    for key, value in pairs(ActivePed:petsList()) do
        openMenu[#openMenu + 1] = {
            ...
            icon = 'fa-solid fa-' .. key,
            params = {
                event = "nmsh-companion:client:switchControl_event",
                args = {
                    index = value.key
                }
            }
...
AddEventHandler('nmsh-companion:client:switchControl_event', function(option)
    if option.index < 0 then
        return
    end
    ActivePed:switchControl(option.index)
    TriggerEvent('nmsh-companion:client:action_menu')
end)
...
RegisterCommand('+showMenu', function()
    ...
    if ((IsDowned() and IsPoliceOrEMS()) or not IsDowned()) and not Ishandcuffed() and not IsPauseMenuActive() and
        ...
        TriggerEvent('nmsh-companion:client:main_menu')
    end
end, false)
...
--           Customization menu
-- =======================================
RegisterNetEvent('nmsh-companion:client:start_grooming_process', function()
    local activePed = ActivePed:read()
    if type(activePed) ~= "table" then
        QBCore.Functions.Notify(Lang:t('error.no_pet_under_control'), 'error', 5000)
        return
    end
    TriggerServerEvent('nmsh-companion:server:grooming_process', activePed.itemData)
end)

RegisterNetEvent('nmsh-companion:client:initialization_process', function(item, pet_information)
    if type(item) ~= "table" then
        QBCore.Functions.Notify(Lang:t('error.failed_to_start_procces'), 'error', 5000)
        return
    end
    if pet_information.type == 'init' then
        TriggerEvent('nmsh-companion:client:openMenu_customization', {
...
AddEventHandler('nmsh-companion:client:openMenu_customization', function(data)
    openMenu_customization(data)
end)

AddEventHandler('nmsh-companion:client:openMenu_customization_rename', function(data)
    ...
    if inputData then
        ...
        if type(validation) == "table" and next(validation) ~= nil then
            ...
            if validation.reason == 'badword' then
                ...
                TriggerEvent('nmsh-companion:client:openMenu_customization_rename', data)
                return
...
AddEventHandler('nmsh-companion:client:openMenu_customization_rename:rename', function(data)
    rename(data)
end)

AddEventHandler('nmsh-companion:client:openMenu_customization_select_variation', function(data)
    ...
    local openMenu = {
        {
            ...
            icon = 'fa-solid fa-angle-left',
            params = {
                event = "nmsh-companion:client:openMenu_customization_select_variation",
                args = {
                    item = item,
...
AddEventHandler('nmsh-companion:client:openMenu_customization_variation:variation_menu', function(data)
...
function openMenu_customization(data)
    ...
    local openMenu = {
        ...
        },
        {
            ...
            params = {
                event = "nmsh-companion:client:openMenu_customization_rename",
                args = {
                    item = data.item,
        ...
        },
        {
            ...
            params = {
                event = "nmsh-companion:client:openMenu_customization:confirm",
                args = {
                    item = data.item,
                    pet_information = data.pet_information
                }
    ...
    local openMenu = {
        {
            ...
            icon = 'fa-solid fa-angle-left',
            params = {
                event = "nmsh-companion:client:openMenu_customization",
                args = {
                    item = item,
                    pet_information = pet_information
                },
        ...
        },
        {
            ...
            icon = 'fa-regular fa-keyboard',
            params = {
                event = "nmsh-companion:client:openMenu_customization_rename:rename",
                args = {
                    item = item,
                    pet_information = pet_information
                },
...
function openMenu_customization_select_variation(data)
    ...
    local openMenu = {
        ...
        },
        {
            ...
            icon = 'fa-solid fa-brush',
            params = {
                event = "nmsh-companion:client:openMenu_customization_variation:variation_menu",
                args = {
                    item = item,
                    pet_information = pet_information
                }
...
AddEventHandler('nmsh-companion:client:openMenu_customization:confirm', function(data)
    TriggerServerEvent('nmsh-companion:server:compelete_initialization_process', data.item, data.pet_information.type)
end)
