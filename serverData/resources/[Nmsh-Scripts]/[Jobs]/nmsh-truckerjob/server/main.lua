...
RegisterNetEvent('nmsh-trucker:server:DoBail', function(bool, vehInfo)
    local src = source
    ...
    if bool then
        if Player.PlayerData.money.cash >= Config.TruckerJobTruckDeposit then
            ...
            TriggerClientEvent('nmsh-trucker:client:SpawnVehicle', src, vehInfo)
...
RegisterNetEvent('nmsh-trucker:server:01101110', function(drops)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
RegisterNetEvent('nmsh-trucker:server:nano', function()
