    ...
    if event == "CEventNetworkEntityDamage" then
        ...
        if victimDied and NetworkGetPlayerIndexFromPed(victim) == PlayerId() and IsEntityDead(PlayerPedId()) then
            ...
            elseif InLaststand and not isDead then
                ...
                TriggerServerEvent("nmsh-log:server:CreateLog", "death",
        ...
        if isDead or InLaststand then
            ...
            if isDead then
                if not isInHospitalBed then
                    if deathTime > 0 then
                        DrawTxt(0.93, 1.44, 1.0, 1.0, 0.6, Lang:t('info.respawn_txt', { deathtime = math.ceil(deathTime) }),
                            255, 255, 255, 255)
                    else
            ...
            elseif InLaststand then
                ...
                if LaststandTime > Config.MinimumRevive then
                    ...
                    DrawTxt(0.845, 1.44, 1.0, 1.0, 0.6, Lang:t('info.bleed_out_help', { time = math.ceil(LaststandTime) }),
                        255, 255, 255, 255)
