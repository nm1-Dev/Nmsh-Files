...
local function FinishMinigame(faults)
    ...
    if Config.Stock[Quality].Current + 1 <= Config.Stock[Quality].Max[Config.MyLevel] then
        TriggerServerEvent('nmsh-hotdogjob:server:UpdateReputation', Quality)
...
local function StartHotdogMinigame()
    PrepareAnim()
    TriggerEvent('nmsh-keyminigame:show')
    TriggerEvent('nmsh-keyminigame:start', FinishMinigame)
end
...
local function StartWorking()
    QBCore.Functions.TriggerCallback('nmsh-hotdogjob:server:HasMoney', function(HasMoney)
        if HasMoney then
            if Config.UseTarget then
                ...
                exports['nmsh-target']:AddTargetEntity(StandObject, {
                    options = {
                        {
                            icon = "fas fa-hand",
...
local function StopWorking()
    if DoesEntityExist(StandObject) then
        QBCore.Functions.TriggerCallback('nmsh-hotdogjob:server:BringBack', function(DidBail)
            if DidBail then
                DeleteObject(StandObject)
                ClearPedTasksImmediately(PlayerPedId())
                IsWorking = false
            ...
            if SellingData.Hotdog ~= nil then
                ...
                if Config.UseTarget then
                    if not zoneMade then
                        zoneMade = true
                        exports['nmsh-target']:AddEntityZone('sellingDogPed', ped, {
                            name = 'sellingDogPed',
                            debugPoly = false,
                        }, {
                            options = {
                                {
                                    ...
                                    action = function(entity)
                                        ...
                                        TriggerServerEvent('nmsh-hotdogjob:server:Sell', pedCoords, HotdogsForSale, SellingPrice)
                                        SellingData.HasTarget = false
                                        LoadAnim('mp_common')
                                        ...
                                        SellingData.Hotdog = nil
                                        exports['nmsh-target']:RemoveZone('sellingDogPed')
                                        zoneMade = false
                                    end,
                                },
                                {
...
RegisterNetEvent('nmsh-hotdogjob:client:UpdateReputation', function(JobRep)
    PlayerData.metadata["jobrep"] = JobRep
    UpdateLevel()
end)

RegisterNetEvent('nmsh-hotdogjob:client:ToggleSell', function()
...
RegisterNetEvent('nmsh-hotdogjob:staff:DeletStand', function()
    local ped = PlayerPedId()
...
CreateThread(function()
    if Config.UseTarget then
        exports['nmsh-target']:AddBoxZone('hotdog_start', vector3(Config.Locations["take"].coords.x, Config.Locations["take"].coords.y, Config.Locations["take"].coords.z), 1, 1, {
            name = 'hotdog_start',
            debugPoly = false,
