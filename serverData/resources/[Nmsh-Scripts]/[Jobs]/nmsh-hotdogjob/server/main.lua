...
QBCore.Functions.CreateCallback('nmsh-hotdogjob:server:HasMoney', function(source, cb)
...
QBCore.Functions.CreateCallback('nmsh-hotdogjob:server:BringBack', function(source, cb)
    local Player = QBCore.Functions.GetPlayer(source)
...
RegisterNetEvent('nmsh-hotdogjob:server:Sell', function(coords, amount, price)
    local src = source
...
RegisterNetEvent('nmsh-hotdogjob:server:UpdateReputation', function(quality)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    ...
    if quality == "exotic" then
        if JobReputation["hotdog"] ~= nil and JobReputation["hotdog"] + 3 > Config.MaxReputation then
            ...
            TriggerClientEvent('nmsh-hotdogjob:client:UpdateReputation', src, JobReputation)
            return
        end
...
QBCore.Commands.Add("removestand", Lang:t("info.command"), {}, false, function(source, _)
    TriggerClientEvent('nmsh-hotdogjob:staff:DeletStand', source)
end, 'admin')
