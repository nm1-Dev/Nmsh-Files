...
function SpawnGoPostal(x, y, z, h)
	...
	exports['nmsh-fuel']:SetFuel(goPostalVehicle, 100.0)
...
CreateThread(function()
	...
	exports['nmsh-target']:AddTargetModel('s_m_m_postal_01', {
		options = {
			{
				type = "client",
				event = "nmsh-postalop:startwork",
			...
			{
				type = "client",
				event = "nmsh-postalop:finishwork",
	...
	exports['nmsh-target']:AddTargetBone('boot', {
		options = {
			{
				type = "client",
				event = "nmsh-postalop:takepackage",
				icon = "fas fa-box",
				label = "Take Package",
				canInteract = function(entity, data)
...
RegisterNetEvent('nmsh-postalop:startwork', function()
	if not isDeliverySignedIn and not onJob then
		if not DoesEntityExist(goPostalVehicle) then
			if QBCore.Functions.GetPlayerData().money.cash >= 1000 then
				...
				TriggerServerEvent('nmsh-delivery:cash', 1000, "remove")
				NewDeliveryShift()
				onJob = true
...
RegisterNetEvent('nmsh-postalop:takepackage', function()
...
RegisterNetEvent('nmsh-postalop:finishwork', function()
	if isDeliverySignedIn then
		if not onJob then
			if DoesEntityExist(goPostalVehicle) then
				...
				if IsVehicleDamaged(goPostalVehicle) then
					TriggerServerEvent('nmsh-delivery:cash', 200, "add")
				else
					TriggerServerEvent('nmsh-delivery:cash', 1000, "add")
				end
				isDeliverySignedIn = false
				onJob = false
				TriggerServerEvent('nmsh-delivery:cash', totalpayamount, "job")
				Wait(500)
				totalpayamount = 0
			else
				isDeliverySignedIn = false
