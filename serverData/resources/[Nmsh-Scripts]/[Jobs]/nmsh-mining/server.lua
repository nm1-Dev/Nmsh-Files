...
RegisterServerEvent('jim-mining:MineReward', function(amount)
    local Player = QBCore.Functions.GetPlayer(source)
    local randomChance = math.random(1, 3)
	-- vector3(-560.24, 2010.72, 127.25)
...
RegisterServerEvent('jim-mining:CrackReward', function(cost)
	local src = source
    local Player = QBCore.Functions.GetPlayer(src)
	if Player.Functions.GetItemByName('stone') then
...
RegisterServerEvent('jim-mining:PanReward', function(amount)
	local src = source
    local Player = QBCore.Functions.GetPlayer(src)
	if not amount then return end
	for i = 1, math.random(1,3) do
