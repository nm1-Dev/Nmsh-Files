...
RegisterNetEvent('nmsh-scrapyard:server:LoadVehicleList', function()
    local src = source
    TriggerClientEvent("nmsh-scapyard:client:setNewVehicles", src, Config.CurrentVehicles)
end)


QBCore.Functions.CreateCallback('nmsh-scrapyard:checkOwnerVehicle', function(_, cb, plate)
...
RegisterNetEvent('nmsh-scrapyard:server:ScrapVehicle', function(listKey)
    local src = source
    ...
    if Config.CurrentVehicles[listKey] ~= nil then
        ...
        TriggerClientEvent("nmsh-scapyard:client:setNewVehicles", -1, Config.CurrentVehicles)
    end
end)
