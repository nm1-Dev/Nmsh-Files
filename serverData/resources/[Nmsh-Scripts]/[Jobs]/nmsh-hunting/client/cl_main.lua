...
AddEventHandler('nmsh-hunting:client:slaughterAnimal', function(entity)
    ...
    CoreName.Functions.Progressbar("harv_anim", "Slaughtering Animal", Config.SlaughteringSpeed, false,
        ...
        if AnimalLootMultiplier:read(entity) ~= nil and AnimalLootMultiplier:read(entity) ~= false then
            TriggerServerEvent('nmsh-hunting:server:AddItem', animal, NetworkGetNetworkIdFromEntity(entity),
                AnimalLootMultiplier:read(entity))
        else
            -- defalut values for multipiler
            TriggerServerEvent('nmsh-hunting:server:AddItem', animal, NetworkGetNetworkIdFromEntity(entity), 'default')
        end
    end)
end)

AddEventHandler('nmsh-hunting:client:sellREQ', function()
    TriggerServerEvent('nmsh-hunting:server:sellmeat')
end)

RegisterNetEvent('nmsh-hunting:client:ForceRemoveAnimalEntity')
AddEventHandler('nmsh-hunting:client:ForceRemoveAnimalEntity', function(entity)
    DeleteEntity(entity)
    ...
    if outPosition.x ~= 0 and outPosition.y ~= 0 and outPosition.z ~= 0 then
        Citizen.CreateThread(function()
            ...
            if startSpawningTimer == 0 then
                ...
                TriggerServerEvent('nmsh-hunting:server:choiceWhichAnimalToSpawn', coord, outPosition, zone_type,
                    indicator)
            else
...
RegisterNetEvent('nmsh-hunting:client:useBait', function()
...
RegisterNetEvent('nmsh-hunting:client:spawnAnimal', function(coord, outPosition, C_animal, was_llegal, indicator)
    ...
    TriggerServerEvent('nmsh-hunting:server:removeBaitFromPlayerInventory')
...
RegisterNetEvent('nmsh-hunting:client:spawnanim', function(model, was_llegal)
...
RegisterNetEvent('nmsh-hunting:client:clearTask')
AddEventHandler('nmsh-hunting:client:clearTask', function()
    local playerPed = PlayerPedId()
