...
function isOwner(entity)
     ...
     local is_owner = nil
     -- await didn't work!
     QBCore.Functions.TriggerCallback('nmsh-oilwell:server:is_employee', function(_is_employee, _is_owner)
...
function createOwnerQbTarget(hash, coord)
     exports['nmsh-target']:RemoveZone("oil-rig-" .. hash)
     Targets.qb_target.oilwell(coord, hash)
end

RegisterNetEvent('nmsh-oilwell:client:remove_oilwell', function(data)
     ...
     TriggerServerEvent('nmsh-oilwell:server:remove_oilwell', oilwell.oilrig_hash)
end)
...
---@param coord table
RegisterNetEvent('nmsh-oilrig:client:clearArea', function(coord)
...
RegisterNetEvent('nmsh-oilrig:client:goOnDuty', function(PlayerJob)
