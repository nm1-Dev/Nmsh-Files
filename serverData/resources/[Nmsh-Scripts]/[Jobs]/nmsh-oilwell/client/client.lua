...
function OilRigs:update(s_res, id)
     ...
     self.data_table[id] = s_res
     QBCore.Functions.TriggerCallback('nmsh-oilwell:server:oilwell_metadata', function(metadata)
          self:syncSpeed(self.data_table[id].entity, metadata.speed)
     end, self.data_table[id].oilrig_hash)
end

function OilRigs:startUpdate(cb)
     QBCore.Functions.TriggerCallback('nmsh-oilrig:server:getNetIDs', function(result)
...
--
RegisterNetEvent('nmsh-oilrig:client:changeRigSpeed', function(qbtarget)
     ...
     QBCore.Functions.TriggerCallback('nmsh-oilwell:server:oilwell_metadata', function(metadata)
          OilRigs:startUpdate(function()
               ...
               if inputData then
                    ...
                    TriggerServerEvent('nmsh-oilrig:server:updateSpeed', inputData, rig.id)
               end
          end)
     end, rig.oilrig_hash)
end)
...
RegisterNetEvent('nmsh-oilrig:client:syncSpeed', function(id, speed)
...
function spawnObjects(model, position)
     TriggerEvent('nmsh-oilrig:client:clearArea', position.coord)
...
RegisterNetEvent('nmsh-oilrig:client:spawn')
AddEventHandler('nmsh-oilrig:client:spawn', function()
     local coords = ChooseSpawnLocation()
     QBCore.Functions.TriggerCallback('nmsh-oilrig:server:createNewOilrig', function(NetId)
          if NetId ~= nil then
               local entity = NetworkGetEntityFromNetworkId(NetId)
               OBJECT = entity
               exports['nmsh-target']:AddEntityZone("oil-rig-" .. entity, entity, {
                    name = "oil-rig-" .. entity,
                    ...
                    options = {
                         {
                              type = "client",
                              event = "nmsh-oilrig:client:enterInformation",
                         ...
                         {
                              type = "client",
                              event = "nmsh-oilwell:menu:OPENMENU",
                              icon = "fa-regular fa-file-lines",
...
RegisterNetEvent('nmsh-oilrig:client:enterInformation', function(qbtarget)
     local inputData = exports['nmsh-input']:ShowInput({
     ...
     if inputData then
          ...
          QBCore.Functions.TriggerCallback('nmsh-oilrig:server:regiserOilrig', function(result)
...
RegisterNetEvent('nmsh-oilwell:client:force_reload', function()
     Wait(25)
     loadData()
end)
...
RegisterNetEvent('nmsh-oilrig:client:local_mail_sender', function(data)
...
RegisterNetEvent('nmsh-oilwell:server_lib:AddExplosion', function(bullding_type)
