...
local function DestroyPickupTarget()
  ...
  if Config.UseTarget then
    exports['nmsh-target']:RemoveZone(pickupTargetID)
    pickupZone = nil
  else
    pickupZone:destroy()
    pickupZone = nil
...
local function RegisterEntranceTarget()
  ...
  if Config.UseTarget then
    entranceZone = exports['nmsh-target']:AddBoxZone(entranceTargetID, coords, 1, 4, {
      ...
      options = {
        {
          type = 'client',
          event = 'nmsh-recyclejob:client:target:enterLocation',
...
local function RegisterExitTarget()
  ...
  if Config.UseTarget then
    exitZone = exports['nmsh-target']:AddBoxZone(exitTargetID, coords, 1, 4, {
      name = exitTargetID,
      heading = 270,
      ...
      options = {
        {
          type = 'client',
          event = 'nmsh-recyclejob:client:target:exitLocation',
...
local function DestroyExitTarget()
  ...
  if Config.UseTarget then
    exports['nmsh-target']:RemoveZone(exitTargetID)
    exitZone = nil
  else
    exitZone:destroy()
    exitZone = nil
...
local function RegisterDutyTarget()
  ...
  if Config.UseTarget then
    dutyZone = exports['nmsh-target']:AddBoxZone(dutyTargetID, coords, 1, 1, {
      name = dutyTargetID,
      heading = 270,
      ...
      options = {
        {
          type = 'client',
          event = 'nmsh-recyclejob:client:target:toggleDuty',
...
local function DestroyDutyTarget()
  ...
  if Config.UseTarget then
    exports['nmsh-target']:RemoveZone(dutyTargetID)
    dutyZone = nil
  else
    dutyZone:destroy()
    dutyZone = nil
...
local function RegisterDeliveyTarget()
  ...
  if Config.UseTarget then
    deliveryZone = exports['nmsh-target']:AddBoxZone(deliveryTargetID, coords, 1, 1, {
      ...
      options = {
        {
          type = 'client',
          event = 'nmsh-recyclejob:client:target:dropPackage',
...
local function DestroyDeliveryTarget()
  ...
  if Config.UseTarget then
    exports['nmsh-target']:RemoveZone(deliveryTargetID)
    deliveryZone = nil
  else
...
function RegisterPickupTarget(coords)
  ...
  if Config.UseTarget then
    pickupZone = exports['nmsh-target']:AddBoxZone(pickupTargetID, targetCoords, 4, 1.5, {
      name = pickupTargetID,
      heading = coords.h,
      minZ = coords.z - 1.0,
      maxZ = coords.z + 2.0,
      ...
      options = {
        {
          type = 'client',
          event = 'nmsh-recyclejob:client:target:pickupPackage',
...
RegisterNetEvent('nmsh-recyclejob:client:target:enterLocation', function()
  EnterLocation()
end)

RegisterNetEvent('nmsh-recyclejob:client:target:exitLocation', function()
  ExitLocation()
end)

RegisterNetEvent('nmsh-recyclejob:client:target:toggleDuty', function()
  onDuty = not onDuty
  if onDuty then
...
RegisterNetEvent('nmsh-recyclejob:client:target:pickupPackage', function()
...
RegisterNetEvent('nmsh-recyclejob:client:target:dropPackage', function()
  ...
  QBCore.Functions.Progressbar('deliver_reycle_package',  Lang:t("text.unpacking_the_package"), Config.DeliveryActionDuration, false, true, {
      ...
      -- Done
      StopAnimTask(PlayerPedId(), 'mp_car_bomb', 'car_bomb_mechanic', 1.0)
      TriggerServerEvent('nmsh-recycle:server:getItem')
      GetRandomPackage()
  end)
end)
    ...
    while true do
      ...
      if isInsideEntranceZone then
        ...
        if IsControlJustReleased(0, 38) then
          ...
          Wait(500)
          TriggerEvent('nmsh-recyclejob:client:target:enterLocation')
          exports['qb-core']:HideText()
        end
      end
      ...
      if isInsideExitZone then
        ...
        if IsControlJustReleased(0, 38) then
          exports['qb-core']:KeyPressed()
          Wait(500)
          TriggerEvent('nmsh-recyclejob:client:target:exitLocation')
          exports['qb-core']:HideText()
        end
      end
      ...
      if isInsideDutyZone then
        ...
        if IsControlJustReleased(0, 38) then
          exports['qb-core']:KeyPressed()
          Wait(500)
          TriggerEvent('nmsh-recyclejob:client:target:toggleDuty')
          exports['qb-core']:HideText()
        end
      end
      ...
      if onDuty then
        if isInsidePickupZone and not carryPackage then
          ...
          if IsControlJustReleased(0, 38) then
            exports['qb-core']:KeyPressed()
            Wait(500)
            TriggerEvent('nmsh-recyclejob:client:target:pickupPackage')
            exports['qb-core']:HideText()
          end
        ...
        if isInsideDeliveryZone and carryPackage then
          ...
          if IsControlJustReleased(0, 38) then
            exports['qb-core']:KeyPressed()
            Wait(500)
            TriggerEvent('nmsh-recyclejob:client:target:dropPackage')
            exports['qb-core']:HideText()
          end
        end
      end
