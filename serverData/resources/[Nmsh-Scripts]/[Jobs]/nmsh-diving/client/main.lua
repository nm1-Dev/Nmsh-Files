...
local function callCops()
    ...
    if call == chance then
        TriggerServerEvent('nmsh-diving:server:CallCops', coords)
    end
end
...
local function takeCoral(coral)
    ...
    QBCore.Functions.Progressbar("take_coral", Lang:t("info.collecting_coral"), times * 1000, false, true, {
        ...
        TriggerServerEvent('nmsh-diving:server:TakeCoral', currentDivingLocation.area, coral, true)
        ClearPedTasks(ped)
...
local function setDivingLocation(divingLocation)
    if currentDivingLocation.area ~= 0 then
        for k in pairs(Config.CoralLocations[currentDivingLocation.area].coords.Coral) do
            if Config.UseTarget then
                exports['nmsh-target']:RemoveZone(k)
            else
    ...
    for k, v in pairs(Config.CoralLocations[currentDivingLocation.area].coords.Coral) do
        if Config.UseTarget then
            exports['nmsh-target']:AddBoxZone('diving_coral_zone_'..k, v.coords, v.length, v.width, {
...
local function sellCoral()
    ...
    QBCore.Functions.Progressbar("sell_coral_items", Lang:t("info.checking_pockets"), math.random(2000, 4000), false, true, {}, {}, {}, {}, function() -- Done
        ...
        TriggerServerEvent('nmsh-diving:server:SellCoral')
...
RegisterNetEvent('QBCore:Client:OnPlayerLoaded', function()
    QBCore.Functions.TriggerCallback('nmsh-diving:server:GetDivingConfig', function(config, area)
...
RegisterNetEvent('nmsh-diving:client:NewLocations', function()
    QBCore.Functions.TriggerCallback('nmsh-diving:server:GetDivingConfig', function(config, area)
        Config.CoralLocations = config
        setDivingLocation(area)
    end)
...
RegisterNetEvent('nmsh-diving:client:UpdateCoral', function(area, coral, bool)
    Config.CoralLocations[area].coords.Coral[coral].PickedUp = bool
end)

RegisterNetEvent('nmsh-diving:client:CallCops', function(coords, msg)
...
RegisterNetEvent("nmsh-diving:client:setoxygenlevel", function()
    if oxgenlevell == 0 then
       ...
       TriggerServerEvent('nmsh-diving:server:removeItemAfterFill')
    else
...
RegisterNetEvent('nmsh-diving:client:UseGear', function()
    local ped = PlayerPedId()
