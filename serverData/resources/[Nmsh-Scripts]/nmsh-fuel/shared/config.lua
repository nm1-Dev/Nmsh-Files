...
Config.FuelTargetExport = false         -- DO NOT USE WITH OX_TARGET! This is only used to fix this nmsh-target issue: https://github.com/CodineDev/cdn-fuel/issues/3. <br> <br> If you don't have this issue and haven't installed this exports in nmsh-target, then this should be false. Otherwise there will be an error.
...
Config.Core =
'qb-core'              -- Change this to your core resources (Ex: 'qbx-core' | 'qb-core'), must be qb based!
Config.Ox = {
    Inventory = false, -- Uses OX_Inventory's metadata instead of nmsh-inventory's.
    Menu = false,      -- Uses OX Libraries instead of nmsh-menu.
    Input = false,     -- Uses Ox Input Dialog instead of nmsh-input.
...
"nmsh-target"           -- Supported: { 'nmsh-target', 'ox_target'} -- Others must use the same format as nmsh-target or manual configuration is required.
