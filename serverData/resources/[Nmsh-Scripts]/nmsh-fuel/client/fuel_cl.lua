...
if Config.RenewedPhonePayment then
	RegisterNetEvent('cdn-fuel:client:phone:PayForFuel', function(amount)
		...
		local success = exports['nmsh-phone']:PhoneNotification(Lang:t("fuel_phone_header"), Lang:t("phone_notification")..total, 'fas fa-gas-pump', '#9f0e63', "NONE", 'fas fa-check-circle', 'fas fa-times-circle')
		if success then
	...
	if Config.Ox.Input then
		...
		if Config.PlayerOwnedGasStationsEnabled and not Config.UnlimitedFuel and not RefuelingType == 'special' then
			if ReserveLevels < maxfuel then
				...
				fuel = exports['nmsh-input']:ShowInput({
...
RegisterNetEvent('cdn-fuel:jerrycan:refuelvehicle', function(data)
	...
	if NotElectric then
		...
		if Config.Ox.Input then
			...
			local refuel = exports['nmsh-input']:ShowInput({
...
local function PoliceAlert(coords)
	...
	if chance < Config.SyphonPoliceCallChance then
		if Config.SyphonDispatchSystem == "ps-dispatch" then
			exports['nmsh-dispatch']:SuspiciousActivity()
		elseif Config.SyphonDispatchSystem == "nmsh-dispatch" then
			TriggerServerEvent('nmsh-dispatch:911call', coords)
		elseif Config.SyphonDispatchSystem == "nmsh-default" then
		...
		if HasSyphon then
			...
			if reason == "syphon" then
				if Config.Ox.Input then
					...
					local syphon = exports['nmsh-input']:ShowInput({
...
CreateThread(function()
	...
	if Config.TargetResource == 'ox_target' then
		...
		exports['nmsh-target']:AddTargetModel(props, {
			options = {
				{
					num = 1,
					type = "client",
