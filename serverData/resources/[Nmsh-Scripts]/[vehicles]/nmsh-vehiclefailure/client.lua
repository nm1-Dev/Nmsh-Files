...
local function DamageRandomComponent()
    ...
    exports['nmsh-mechanicjob']:SetVehicleStatus(QBCore.Functions.GetPlate(vehicle), randomComponent, exports['nmsh-mechanicjob']:GetVehicleStatus(QBCore.Functions.GetPlate(vehicle), randomComponent) - randomDamage)
end
...
local function CleanVehicle(veh)
    ...
    QBCore.Functions.Progressbar("cleaning_vehicle", Lang:t("progress.clean_veh"), math.random(10000, 20000), false, true, {
        ...
        TriggerServerEvent('nmsh-vehiclefailure:server:removewashingkit', veh)
...
local function RepairVehicleFull(veh)
    ...
    QBCore.Functions.Progressbar("repair_vehicle", Lang:t("progress.repair_veh"), math.random(20000, 30000), false, true, {
        ...
        for i = 0, 5 do
            ...
            TriggerEvent('nmsh-vehiclefailure:client:TyreSync', veh, i)
        end
        if (IsBackEngine(GetEntityModel(veh))) then
        ...
        TriggerServerEvent('nmsh-vehiclefailure:removeItem', "advancedrepairkit")
    end, function() -- Cancel
        StopAnimTask(PlayerPedId(), "mini@repair", "fixing_a_player", 1.0)
        QBCore.Functions.Notify(Lang:t("error.failed_notification"), "error")
        if (IsBackEngine(GetEntityModel(veh))) then
...
local function RepairVehicle(veh)
    ...
    QBCore.Functions.Progressbar("repair_vehicle", Lang:t("progress.repair_veh"), math.random(10000, 20000), false, true, {
        ...
        TriggerServerEvent('nmsh-vehiclefailure:removeItem', "repairkit")
    end, function() -- Cancel
        StopAnimTask(PlayerPedId(), "mini@repair", "fixing_a_player", 1.0)
        QBCore.Functions.Notify(Lang:t("error.failed_notification"), "error")
        if (IsBackEngine(GetEntityModel(veh))) then
...
RegisterNetEvent('nmsh-vehiclefailure:client:RepairVehicle', function()
...
RegisterNetEvent('nmsh-vehiclefailure:client:SyncWash', function(veh)
    SetVehicleDirtLevel(veh, 0.1)
    SetVehicleUndriveable(veh, false)
    WashDecalsFromVehicle(veh, 1.0)
end)

RegisterNetEvent('nmsh-vehiclefailure:client:CleanVehicle', function()
    local veh = QBCore.Functions.GetClosestVehicle()
...
RegisterNetEvent('nmsh-vehiclefailure:client:RepairVehicleFull', function()
    local veh = QBCore.Functions.GetClosestVehicle()
    if veh ~= nil and veh ~= 0 then
        local ped = PlayerPedId()
        local pos = GetEntityCoords(ped)
...
RegisterNetEvent('nmsh-vehiclefailure:client:TyreSync', function(veh, tyre)
                    ...
                    if cfg.sundayDriver and GetVehicleClass(vehicle) ~= 14 then -- Not for boats
                        ...
                        if speed >= 1.0 then
                            ...
                            if brake > 127 then
                                ...
                                --exports['nmsh-vehicletuning']:SetVehicleStatus(QBCore.Functions.GetPlate(vehicle), "brakes", exports['nmsh-vehicletuning']:GetVehicleStatus(QBCore.Functions.GetPlate(vehicle), "brakes") - 0.01)
                            end
