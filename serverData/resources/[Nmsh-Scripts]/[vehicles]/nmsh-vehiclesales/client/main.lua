        ...
        if vehicles then
            for i = 1, #vehicles, 1 do
                ...
                if Config.UseTarget then
                    ...
                    EntityZones[i] = exports['nmsh-target']:AddTargetEntity(occasionVehicles[Zone][i].car, {
                        options = {
                            {
                                type = "client",
                                event = "nmsh-vehiclesales:client:OpenContract",
                                icon = "fas fa-car",
...
local function despawnOccasionsVehicles()
    ...
    for i = 1, #oSlot, 1 do
        ...
        if EntityZones[i] and Config.UseTarget then
            exports['nmsh-target']:RemoveZone(EntityZones[i])
        end
    end
    EntityZones = {}
end
...
local function SellData(data, model)
    QBCore.Functions.TriggerCallback("nmsh-vehiclesales:server:CheckModelName",function(DataReturning)
        local vehicleData = {}
        ...
        TriggerServerEvent('nmsh-occasions:server:sellVehicle', data.price, vehicleData)
        ...
        while listen do
            if IsControlJustReleased(0, 38) then -- E
                if spot then
                    ...
                    TriggerEvent('nmsh-vehiclesales:client:OpenContract', data)
                else
                    if IsPedInAnyVehicle(PlayerPedId(), false) then
                        listen = false
                        TriggerEvent('nmsh-occasions:client:MainMenu')
                        --TriggerEvent('nmsh-vehiclesales:client:SellVehicle')
                    else
    ...
    for k, v in pairs(Config.Zones) do
        ...
        SellSpot:onPlayerInOut(function(isPointInside)
            if isPointInside and Zone ~= k then
                Zone = k
                QBCore.Functions.TriggerCallback('nmsh-occasions:server:getVehicles', function(vehicles)
...
RegisterNUICallback('buyVehicle', function(_, cb)
    TriggerServerEvent('nmsh-occasions:server:buyVehicle', CurrentVehicle)
    cb('ok')
end)

RegisterNUICallback('takeVehicleBack', function(_, cb)
    TriggerServerEvent('nmsh-occasions:server:ReturnVehicle', CurrentVehicle)
    cb('ok')
end)

-- Events

RegisterNetEvent('nmsh-occasions:client:BuyFinished', function(vehdata)
...
RegisterNetEvent('nmsh-occasions:client:SellBackCar', function()
    ...
    if IsPedInAnyVehicle(ped, false) then
        ...
        QBCore.Functions.TriggerCallback('nmsh-garage:server:checkVehicleOwner', function(owned, balance)
            if owned then
                if balance < 1 then
                    TriggerServerEvent('nmsh-occasions:server:sellVehicleBack', vehicleData)
...
RegisterNetEvent('nmsh-occasions:client:ReturnOwnedVehicle', function(vehdata)
    local vehmods = json.decode(vehdata.mods)
    DoScreenFadeOut(250)
    Wait(500)
    QBCore.Functions.TriggerCallback('QBCore:Server:SpawnVehicle', function(netId)
...
RegisterNetEvent('nmsh-occasion:client:refreshVehicles', function()
    if Zone then
        QBCore.Functions.TriggerCallback('nmsh-occasions:server:getVehicles', function(vehicles)
            despawnOccasionsVehicles()
            spawnOccasionsVehicles(vehicles)
...
RegisterNetEvent('nmsh-vehiclesales:client:SellVehicle', function()
...
RegisterNetEvent('nmsh-vehiclesales:client:OpenContract', function(data)
    CurrentVehicle = occasionVehicles[Zone][data.Contract]
    if CurrentVehicle then
        QBCore.Functions.TriggerCallback('nmsh-occasions:server:getSellerInformation', function(info)
            if info then
...
RegisterNetEvent('nmsh-occasions:client:MainMenu', function()
    local MainMenu = {
        {
            isMenuHeader = true,
        ...
        },
        {
            ...
            params = {
                event = 'nmsh-vehiclesales:client:SellVehicle',
            }
        },
        {
            ...
            params = {
                event = 'nmsh-occasions:client:SellBackCar',
            }
        }
    }
