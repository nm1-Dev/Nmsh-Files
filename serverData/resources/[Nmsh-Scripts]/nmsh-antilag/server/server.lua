...
RegisterNetEvent("nmsh-antilag:syncflames:s", function(entity,enb)
    if type(entity) ~= "number" then return end
    TriggerClientEvent("nmsh-antilag:syncflames:c", -1, entity,enb)
end)
...
Wrapper.CreateCallback('nmsh-antilag:check', function(source, cb,_plate)
    local src = source
...
RegisterNetEvent('nmsh-antilag:install:s',function(_plate)
    local src = source
    local plate = _plate
...
RegisterNetEvent('nmsh-nos:removenos',function(_plate)
    local src = souce
    local plate = _plate
