        ...
        if v.Type == "installer" then 
            QBCore.Functions.CreateUseableItem(v.Name, function(source, item)
                ...
                if Player.Functions.GetItemByName(item.name) then
                    ...
                    TriggerClientEvent('nmsh-antilag:noscam:install',src,item.name)
                    return
                end
            end)
        end
        ...
        if v.Type == "remover" then 
            QBCore.Functions.CreateUseableItem(v.Name, function(source, item)
                ...
                if Player.Functions.GetItemByName(item.name) then
                    ...
                    TriggerClientEvent('nmsh-antilag:noscam:remove',src,item.name)
                    return
                end
            end)
        end
    ...
    for k,v in pairs(Config.Items) do
        if v.Type == "installer" then 
            ...
            ESX.RegisterUsableItem(v.Name, function(source)
                local src = source
                TriggerEvent('Wrapper:AntiLag:Log', src, 'Used AntiLag System Item')
                TriggerClientEvent('nmsh-antilag:noscam:install', src, Item) 
                return
            end)
        end
    end
    for k,v in pairs(Config.Items) do
        if v.Type == "remover" then 
            ...
            ESX.RegisterUsableItem(v.Name, function(source)
                local src = source
                TriggerEvent('Wrapper:AntiLag:Log', src, 'Used Antilag Remove Item')
                TriggerClientEvent('nmsh-antilag:noscam:remove', src, Item) 
                return
            end)
        end
    end
                ...
                for k,v in pairs(Config.Settings.Commands.Allowed) do 
                    if v == myid.steam or Config.Settings.Commands.Permissions == false then 
                        if Config.Settings.Commands.Enabled then 
                            ...
                            TriggerClientEvent('nmsh-antilag:noscam:install',src)
                            return
                        end
                    end
                end
                ...
                for k,v in pairs(Config.Settings.Commands.Allowed) do 
                    -- if v == myid.steam or Config.Settings.Commands.Permissions == false then 
                        if Config.Settings.Commands.Enabled then 
                            ...
                            TriggerClientEvent('nmsh-antilag:noscam:remove',src)
                            return
                        end
                    -- end
                end
