                    ...
                    if gear ~= kp  then
                        if not IsEntityInAir(veh) then
                            if not IsControlPressed(1, 71) and not IsControlPressed(1, 72) then
                                ...
                                TriggerServerEvent("nmsh-antilag:syncflames:s", VehToNet(veh),true)
                                TriggerServerEvent("sound_server:PlayWithinDistance", 25.0,tostring(math.random(1, 6)),0.9)
                                SetVehicleTurboPressure(veh, 25)
                                Wait(Main.Delay)
                                TriggerServerEvent("nmsh-antilag:syncflames:s", VehToNet(veh),false)
                            end
                        end
                    end
                end
...
RegisterNetEvent("nmsh-antilag:syncflames:c", function(vehicle,enabled)
...
function Main:HasStep(_plate)
    local plate = _plate
    Wrapper.TriggerCallback('nmsh-antilag:check', function(data)
        returnval = data
    end,plate)
    Wait(1000)
    return returnval
...
RegisterNetEvent('nmsh-baseevents:enteredVehicle:2step',function(veh,_plate)
    local plate = _plate
    Wrapper.TriggerCallback('nmsh-antilag:check', function(data)
        Main.antilag = data
    end,plate)
...
RegisterNetEvent('nmsh-baseevents:leftVehicle:2step',function()
    Main.antilag = false
end)

RegisterNetEvent('nmsh-antilag:noscam:install',function(_item)
    ...
    if closestDistance < Config.Settings.InstallDist then
            ...
            Wait(1500)
            exports['nmsh-interactbutton']:button('Install Anti-Lag','Anti-Lag installed','nmsh-antilag:install:c',6000)
        else
            Wrapper:Notify(Lang.NoVeh,'error',2500)
        end
end)

RegisterNetEvent('nmsh-antilag:install:c',function()
    local plate = Main:trim(GetVehicleNumberPlateText(installveh))
    TriggerServerEvent('nmsh-antilag:install:s',plate)
...
RegisterNetEvent('nmsh-antilag:noscam:remove',function(_item)
    local closestVehicle,closestDistance = _GetClosestVehicle(GetEntityCoords(PlayerPedId()))
    if closestDistance < Config.Settings.InstallDist then 
        if Main:HasStep(Main:trim(GetVehicleNumberPlateText(closestVehicle))) then 
            Wrapper:RemoveItem(_item,1)
            TriggerServerEvent('nmsh-nos:removenos',Main:trim(GetVehicleNumberPlateText(closestVehicle)))
