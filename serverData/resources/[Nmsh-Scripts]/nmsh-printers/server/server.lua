...
QBCore.Functions.CreateUseableItem("printerdocument", function(source, item)
    TriggerClientEvent('nmsh-printers:client:UsePaperDocument', source, item)
end)

RegisterNetEvent('nmsh-printers:server:SavePaperDocument', function(url)
    local src = source
        ...
        if validexts[extension] then
            ...
            if cash >= Config.Amount then 
                if Player.Functions.RemoveItem('paper', 1) then
                    ...
                    TriggerClientEvent('inventory:client:ItemBox', src, QBCore.Shared.Items['paper'], "remove")
