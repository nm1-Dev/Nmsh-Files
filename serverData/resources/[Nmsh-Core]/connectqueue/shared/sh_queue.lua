...
Queue.MaxPlayers = GetConvarInt("sv_maxclients", 100)
...
QBCore.Commands.Add("qpos", "Set queue pos", {{name="currentpos", help="current pos"}, {name="newpos", help="NEW POSITION"}}, true, function(source, args)
    ...
    if data then 
        ...
        Queue:DebugPrint(""..GetPlayerName(src).." HAVE SET " .. data.name .. " QUEUE POSITION TO: " .. args[2])
    end
end, "god")

RegisterNetEvent('Queue:pushPlayer', function(oldpos, newpos)
    if not oldpos or not newpos then return end
    local src = source

    oldpos, newpos = tonumber(oldpos), tonumber(newpos)

    local data = Queue:GetQueueList()[oldpos]
    if data then 
        Queue:SetPos(data.ids, newpos)
    
        Queue:DebugPrint(""..GetPlayerName(src).." HAVE SET " .. data.name .. " QUEUE POSITION TO: " .. newpos)
    end
end)
