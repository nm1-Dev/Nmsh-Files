...
function QBCore.Functions.TimeFormate(ms)
    ...
    if days > 0 then
        remainingTime = remainingTime .. days .. "d "
        hours = hours % 24
    end
    ...
    if minutes > 0 then
        remainingTime = remainingTime .. minutes .. "m "
        seconds = seconds % 60
    end

    remainingTime = remainingTime .. seconds .. "s"
...
function QBCore.Functions.AddPermission(source, permission)
    ...
    if Player then
        QBCore.Config.Server.Permissions[plicense] = {
            license = plicense,
        ...
        ExecuteCommand(('add_principal identifier.%s qbcore.%s'):format(plicense, permission))
