    ...
    if source and source ~= '' then
        if citizenid then
            ...
            if PlayerData and license == PlayerData.license then
                ...
                TriggerEvent('nmsh-log:server:CreateLog', 'anticheat', 'Anti-Cheat', 'white',
...
function QBCore.Player.CheckPlayerData(source, PlayerData)
    ...
    PlayerData.metadata['imadminjail'] = PlayerData.metadata['imadminjail'] or 0
    PlayerData.metadata['jailitems'] = PlayerData.metadata['jailitems'] or {}
    PlayerData.metadata['gameitems'] = PlayerData.metadata['gameitems'] or {}
    PlayerData.metadata['fightitems'] = PlayerData.metadata['fightitems'] or {}
    ...
    PlayerData.metadata["communityservice"] = PlayerData.metadata["communityservice"] ~= nil and
    PlayerData.metadata["communityservice"] or 0
    ...
    PlayerData.items = GetResourceState('nmsh-inventory') ~= 'missing' and
    exports['nmsh-inventory']:LoadInventory(PlayerData.source, PlayerData.citizenid) or {}
...
function QBCore.Player.CreatePlayer(PlayerData, Offline)
    ...
    function self.Functions.AddMoney(moneytype, amount, reason)
        ...
        if not self.Offline then
            ...
            if amount > 100000 then
                TriggerEvent('nmsh-log:server:CreateLog', 'playermoney', 'AddMoney', 'lightgreen',
                    '**' ..
    ...
    function self.Functions.RemoveMoney(moneytype, amount, reason)
        ...
        if not self.Offline then
            self.Functions.UpdatePlayerData()
            if amount > 100000 then
                TriggerEvent('nmsh-log:server:CreateLog', 'playermoney', 'RemoveMoney', 'red',
                    '**' ..
                    GetPlayerName(self.PlayerData.source) ..
                    ' (citizenid: ' ..
                    self.PlayerData.citizenid ..
    ...
    function self.Functions.SetMoney(moneytype, amount, reason)
        ...
        if not self.Offline then
            self.Functions.UpdatePlayerData()
            TriggerEvent('nmsh-log:server:CreateLog', 'playermoney', 'SetMoney', 'green',
                '**' ..
                GetPlayerName(self.PlayerData.source) ..
                ' (citizenid: ' ..
                self.PlayerData.citizenid ..
...
function QBCore.Player.DeleteCharacter(source, citizenid)
    ...
    if license == result then
        ...
        MySQL.transaction(queries, function(result2)
            if result2 then
                TriggerEvent('nmsh-log:server:CreateLog', 'joinleave', 'Character Deleted', 'red',
...
function QBCore.Player.ForceDeleteCharacter(citizenid)
    ...
    if result then
        ...
        MySQL.transaction(queries, function(result2)
            if result2 then
                TriggerEvent('nmsh-log:server:CreateLog', 'joinleave', 'Character Force Deleted', 'red',
